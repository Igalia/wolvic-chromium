...
void SelectPopup::ShowMenu(
  ...
  ScopedJavaLocalRef<jobject> j_obj = java_obj_.get(env);
  if (j_obj.is_null()){
    LOG(ERROR) << "R800ZZ_SELECT: j_obj is null"; //r800zz
    return;
  }
...
/* r800zz wolvic-chromium SELECT POPUP bugfix delete
  if (popup_view.is_null())
    return;
*/
  //r800zz wolvic-chromium SELECT POPUP bug fix add
  // Wolvic uses its own SelectPopup.Factory UI and does not require an Android
  // anchor View. //r800zz
  if (!popup_view.is_null()) { //r800zz
    gfx::RectF bounds_dip = gfx::RectF(bounds); //r800zz
    bounds_dip.Scale(1 / web_contents_->GetNativeView()->GetDipScale()); //r800zz
    view->SetAnchorRect(popup_view, bounds_dip); //r800zz
  } //r800zz
//
...
/* r800zz wolvic-chromium SELECT POPUP fix delete 
