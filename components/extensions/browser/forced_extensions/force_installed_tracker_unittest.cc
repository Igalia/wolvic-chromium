...
TEST_F(ForceInstalledTrackerTest, EmptyForcelistAndThenUpdated) {
  ...
  force_installed_tracker()->OnExtensionLoaded(browser_context(), ext1.get());
  force_installed_tracker()->OnExtensionLoaded(browser_context(), ext2.get());
...
TEST_F(ForceInstalledTrackerTest, AllExtensionsInstalled) {
  ...
  force_installed_tracker()->OnExtensionReady(browser_context(), ext1.get());
  force_installed_tracker()->OnExtensionReady(browser_context(), ext2.get());
  EXPECT_TRUE(loaded_called_);
