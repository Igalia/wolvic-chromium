...
#include "components/extensions/browser/forced_extensions/force_installed_test_base.h"

#include "base/memory/raw_ptr.h"
#include "base/strings/strcat.h"
#include "base/strings/utf_string_conversions.h"
#include "base/values.h"
// #include "components/extensions/browser/external_provider_impl.h"
...
void ForceInstalledTestBase::SetUp() {
  ...
  // TODO(mshin): Support Policy Service
  ...
  prefs_ = env_->GetTestingPrefService();
  registry_ = ExtensionRegistry::Get(browser_context_.get());
  install_stage_tracker_ = InstallStageTracker::Get(browser_context_.get());
  force_installed_tracker_ =
      std::make_unique<ForceInstalledTracker>(registry_, browser_context_.get());
}
...
void ForceInstalledTestBase::SetupForceList(ExtensionOrigin origin) {
  ...
  base::Value::Dict dict =
      base::Value::Dict()
          // TODO(mshin): Enable below code after migrating ExternalProviderImpl
          .Set(kExtensionId1,
               base::Value::Dict().Set("external_update_url"/*ExternalProviderImpl::kExternalUpdateUrl*/,
                                       update_url))
          .Set(kExtensionId2,
               base::Value::Dict().Set("external_update_url"/*ExternalProviderImpl::kExternalUpdateUrl*/,
                                       update_url));
...
scoped_refptr<const Extension> ForceInstalledTestBase::CreateNewExtension(
  ...
  switch (status) {
    ...
    case ForceInstalledTracker::ExtensionStatus::kLoaded:
      ...
      force_installed_tracker()->OnExtensionLoaded(browser_context(), ext.get());
      break;
    case ForceInstalledTracker::ExtensionStatus::kReady:
      registry()->AddEnabled(ext.get());
      force_installed_tracker()->OnExtensionLoaded(browser_context(), ext.get());
      force_installed_tracker()->OnExtensionReady(browser_context(), ext.get());
  }
  return ext;
}
