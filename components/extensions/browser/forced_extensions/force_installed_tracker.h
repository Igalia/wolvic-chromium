...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_FORCED_EXTENSIONS_FORCE_INSTALLED_TRACKER_H_
#define COMPONENTS_EXTENSIONS_BROWSER_FORCED_EXTENSIONS_FORCE_INSTALLED_TRACKER_H_
...
class ForceInstalledTracker : public extensions::ExtensionRegistryObserver,
 ...
 public:
  ...
  ForceInstalledTracker(extensions::ExtensionRegistry* registry,
  ...
  void OnExtensionDownloadCacheStatusRetrieved(
      const extensions::ExtensionId& id,
      extensions::ExtensionDownloaderDelegate::CacheStatus cache_status) override;
  ...
  enum class ExtensionStatus {
    // extensions::Extension appears in force-install list, but it not installed yet.
    kPending,

    // extensions::Extension was successfully loaded.
    kLoaded,

    // extensions::Extension is ready. This happens after loading.
    kReady,

    // extensions::Extension installation failure was reported.
    kFailed
  };
  ...
  const std::map<extensions::ExtensionId, ExtensionInfo>& extensions() const {
    return extensions_;
  }
  ...
  static bool IsExtensionFetchedFromCache(
      const std::optional<extensions::ExtensionDownloaderDelegate::CacheStatus>& status);
 ...
 private:
  ...
  // to the collection.
  void AddExtensionInfo(const extensions::ExtensionId& extension_id,
                        ExtensionStatus status,
                        bool is_from_store);

  // Modifies `extensions_` and bounded counter by changing status
  // of one extension.
  void ChangeExtensionStatus(const extensions::ExtensionId& extension_id,
                             ExtensionStatus status);
  ...
  // this collection.
  std::map<extensions::ExtensionId, ExtensionInfo> extensions_;
  ...
  // TODO(mshin): Change the below status after supporting Policy
  Status status_ = kWaitingForExtensionLoads; // kWaitingForPolicyService;
  bool forced_extensions_pref_ready_ = false;
  PrefChangeRegistrar pref_change_registrar_;

  base::ScopedObservation<extensions::ExtensionRegistry, ExtensionRegistryObserver>
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_FORCED_EXTENSIONS_FORCE_INSTALLED_TRACKER_H_
