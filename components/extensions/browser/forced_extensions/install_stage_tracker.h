...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_FORCED_EXTENSIONS_INSTALL_STAGE_TRACKER_H_
#define COMPONENTS_EXTENSIONS_BROWSER_FORCED_EXTENSIONS_INSTALL_STAGE_TRACKER_H_
...
class InstallStageTracker : public KeyedService {
 public:
  ...
  // ExtensionInstallReportLogEvent::extensions::InstallationStage) when adding new entries.
  ...
  enum class FailureReason {
    ...
    // Corresponds to extensions::CrxInstallErrorType.
  ...
  struct InstallationData {
    ...
    std::optional<extensions::ExtensionDownloaderDelegate::Stage> downloading_stage;
    std::optional<extensions::ExtensionDownloaderDelegate::CacheStatus>
        downloading_cache_status;
    std::optional<FailureReason> failure_reason;
    std::optional<extensions::CrxInstallErrorDetail> install_error_detail;
    ...
    std::optional<extensions::SandboxedUnpackerFailureReason> unpacker_failure_reason;
    // Type of extension, assigned during CRX installation process.
    std::optional<extensions::Manifest::Type> extension_type;
    ...
    std::optional<extensions::ManifestInvalidError> manifest_invalid_error;
    ...
    // See extensions::InstallationStage enum.
    std::optional<extensions::InstallationStage> installation_stage;
  ...
  class Observer : public base::CheckedObserver {
   public:
    ...
    virtual void OnExtensionInstallationFailed(const extensions::ExtensionId& id,
    ...
    virtual void OnExtensionInstallationStageChanged(const extensions::ExtensionId& id,
                                                     Stage stage) {}
  ...
  void ReportInfoOnNoUpdatesFailure(const extensions::ExtensionId& id,
  ...
  void ReportManifestInvalidFailure(
      const extensions::ExtensionId& id,
      const extensions::ExtensionDownloaderDelegate::FailureData& failure_data);

  // Remembers failure reason and in-progress stages in memory.
  void ReportInstallationStage(const extensions::ExtensionId& id, Stage stage);
  void ReportInstallCreationStage(const extensions::ExtensionId& id,
                                  InstallCreationStage stage);
  ...
  void ReportFetchError(
      ...
      FailureReason reason,
      const extensions::ExtensionDownloaderDelegate::FailureData& failure_data);
  void ReportFailure(const extensions::ExtensionId& id, FailureReason reason);
  void ReportDownloadingStage(const extensions::ExtensionId& id,
                              extensions::ExtensionDownloaderDelegate::Stage stage);
  void ReportCRXInstallationStage(const extensions::ExtensionId& id,
                                  extensions::InstallationStage stage);
  ...
  void ReportExtensionType(const extensions::ExtensionId& id,
                           extensions::Manifest::Type extension_type);
  void ReportCrxInstallError(const extensions::ExtensionId& id,
                             FailureReason reason,
                             extensions::CrxInstallErrorDetail crx_install_error);
  void ReportSandboxedUnpackerFailureReason(
      const extensions::ExtensionId& id,
      const extensions::CrxInstallError& crx_install_error);

  // Retrieves known information for installation of extension `id`.
  // Returns empty data if not found.
  InstallationData Get(const extensions::ExtensionId& id);
 ...
 private:
  ...
  void NotifyObserversOfFailure(const extensions::ExtensionId& id,
                                FailureReason reason,
                                const InstallationData& data);

  raw_ptr<const content::BrowserContext> browser_context_;

  std::map<extensions::ExtensionId, InstallationData> installation_data_map_;
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_FORCED_EXTENSIONS_INSTALL_STAGE_TRACKER_H_
