...
#include "components/extensions/browser/cws_item_service.pb.h"
...
CWSInfoServiceTest::CWSInfoServiceTest()
  ...
  env_->GetTestingPrefService()->SetInteger(
        pref_names::kExtensionUnpublishedAvailability, 1);
    
  extension_prefs_ = ExtensionPrefs::Get(browser_context_.get());
  extension_registry_ = ExtensionRegistry::Get(browser_context_.get());

  // Create CWSInfoService instance.
  cws_info_service_ = CWSInfoService::Get(browser_context_.get());
  ...
  auto* test_extension_system = static_cast<TestExtensionSystem*>(
