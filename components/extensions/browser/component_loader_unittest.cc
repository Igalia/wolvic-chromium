...
class ComponentLoaderTest : public ExtensionServiceUserTestBase {
 public:
  void SetUp() override {
    ...
    extension_system_ = static_cast<TestExtensionSystem*>(
        ExtensionSystem::Get(testing_browser_context()));
    ...
    ASSERT_TRUE(base::ReadFileToString(
        extension_path_.Append(extensions::kManifestFilename),
        &manifest_contents_));
  }
 ...
 protected:
  ...
  void RunEmitUserHistogramsTest(int nonuser_expected_total_count,
    ...
    service_->component_loader()->set_profile_for_testing(testing_browser_context());
...
TEST_F(ComponentLoaderTest, AddWhenNotReady) {
  ...
  ExtensionRegistry* registry = ExtensionRegistry::Get(testing_browser_context());
...
// TODO(mshin): Enable the below test after supporting the multiple profile
TEST_F(ComponentLoaderTest, DISABLED_LoadAll_NonUserEmitHistograms) {
