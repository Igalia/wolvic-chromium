...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_MANAGEMENT_H_
#define COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_MANAGEMENT_H_
...
class ExtensionManagement : public KeyedService {
 public:
  ...
  explicit ExtensionManagement(content::BrowserContext* browser_context);
  ...
  const std::vector<std::unique_ptr<extensions::ManagementPolicy::Provider>>& GetProviders()
      const;
  ...
  InstallationMode GetInstallationMode(const extensions::Extension* extension);

  // Returns installation mode for an extension with id |extension_id| and
  // updated with |update_url|.
  InstallationMode GetInstallationMode(const extensions::ExtensionId& extension_id,
  ...
  bool IsUpdateUrlOverridden(const extensions::ExtensionId& id);
  ...
  bool UpdatesFromWebstore(const extensions::Extension& extension);

  // Returns if an extension with id |id| is explicitly allowed by enterprise
  // policy or not.
  bool IsInstallationExplicitlyAllowed(const extensions::ExtensionId& id);

  // Returns if an extension with id |id| is explicitly blocked by enterprise
  // policy or not.
  bool IsInstallationExplicitlyBlocked(const extensions::ExtensionId& id);
  ...
  bool IsAllowedManifestVersion(const extensions::Extension* extension);

  bool IsAllowedByUnpublishedAvailabilityPolicy(const extensions::Extension* extension);

  // Returns the list of blocked API permissions for |extension|.
  extensions::APIPermissionSet GetBlockedAPIPermissions(const extensions::Extension* extension);

  // Returns the list of blocked API permissions for an extension with id
  // |extension_id| and updated with |update_url|.
  extensions::APIPermissionSet GetBlockedAPIPermissions(const extensions::ExtensionId& extension_id,
                                            const std::string& update_url);

  // Returns the list of hosts blocked by policy for |extension|.
  const extensions::URLPatternSet& GetPolicyBlockedHosts(const extensions::Extension* extension);

  // Returns the hosts exempted by policy from the PolicyBlockedHosts for
  // |extension|.
  const extensions::URLPatternSet& GetPolicyAllowedHosts(const extensions::Extension* extension);
  ...
  const extensions::URLPatternSet& GetDefaultPolicyBlockedHosts() const;
  ...
  const extensions::URLPatternSet& GetDefaultPolicyAllowedHosts() const;
  ...
  bool UsesDefaultPolicyHostRestrictions(const extensions::Extension* extension);

  // Returns blocked permission set for |extension|.
  std::unique_ptr<const extensions::PermissionSet> GetBlockedPermissions(
  ...
  const std::string BlockedInstallMessage(const extensions::ExtensionId& id);
  ...
  bool CheckMinimumVersion(const extensions::Extension* extension,
  ...
  // aren't deferred).
  extensions::ExtensionIdSet GetForcePinnedList() const;

  // Returns if an extension with |id| can navigate to file URLs.
  bool IsFileUrlNavigationAllowed(const extensions::ExtensionId& id);
 ...
 private:
  ...
  // |id| yet.
  internal::IndividualSettings* AccessById(const extensions::ExtensionId& id);
  ...
  std::vector<std::unique_ptr<extensions::ManagementPolicy::Provider>> providers_;
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_MANAGEMENT_H_
