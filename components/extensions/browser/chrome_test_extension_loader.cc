...
using extensions::ContentScriptsInfo;
using extensions::ContentScriptLoadWaiter;
using extensions::CrxInstallError;
using extensions::Extension;
using extensions::ExtensionsBrowserClient;
using extensions::ExtensionBackgroundPageWaiter;
...
using extensions::ExtensionUserScriptLoader;
...
bool ChromeTestExtensionLoader::WaitForExtensionReady(
  ...
  content::BrowserContext* context_to_use =
      IncognitoInfo::IsSplitMode(&extension)
          ...
          : ExtensionsBrowserClient::Get()->GetOriginalContext(browser_context_.get());
...
void ChromeTestExtensionLoader::CheckPermissions(const Extension* extension) {
  ...
  if (!allow_file_access_) {
    allow_file_access_ =
        extensions::Manifest::ShouldAlwaysAllowFileAccess(extension->location());
  }
  ...
  // the reload.
  if (*allow_file_access_ != extensions::util::AllowFileAccess(id, browser_context_)) {
  ...
  if (allow_incognito_access_.has_value() &&
      *allow_incognito_access_ !=
          extensions::util::IsIncognitoEnabled(id, browser_context_)) {
    TestExtensionRegistryObserver registry_observer(extension_registry_, id);
...
bool ChromeTestExtensionLoader::CheckInstallWarnings(
  ...
  for (const InstallWarning& warning : install_warnings) {
    ...
    if (warning.message == extensions::manifest_errors::kManifestV2IsDeprecatedWarning)
      continue;
