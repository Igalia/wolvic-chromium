...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_CHROME_CONTENT_VERIFIER_DELEGATE_H_
#define COMPONENTS_EXTENSIONS_BROWSER_CHROME_CONTENT_VERIFIER_DELEGATE_H_
...
class ChromeContentVerifierDelegate : public extensions::ContentVerifierDelegate {
 public:
  ...
  VerifierSourceType GetVerifierSourceType(const extensions::Extension& extension) override;
  extensions::ContentVerifierKey GetPublicKey() override;
  GURL GetSignatureFetchUrl(const extensions::ExtensionId& extension_id,
                            const base::Version& version) override;
  std::set<base::FilePath> GetBrowserImagePaths(
      const extensions::Extension* extension) override;
  void VerifyFailed(const extensions::ExtensionId& extension_id,
                    extensions::ContentVerifyJob::FailureReason reason) override;
  ...
  class GetVerifyInfoTestOverride {
   public:
    using VerifyInfoCallback =
        base::RepeatingCallback<VerifyInfo(const extensions::Extension& extension)>;
 ...
 private:
  ...
  bool IsFromWebstore(const extensions::Extension& extension) const;

  // Returns information needed for content verification of |extension|.
  VerifyInfo GetVerifyInfo(const extensions::Extension& extension) const;
  ...
  std::map<extensions::ExtensionId, std::unique_ptr<net::BackoffEntry>>
  ...
  std::set<extensions::ExtensionId> would_be_disabled_ids_;
  ...
  std::set<extensions::ExtensionId> would_be_reinstalled_ids_;
};

}  // namespace components_extensions

#endif  // COMPONENTS_EXTENSIONS_BROWSER_CHROME_CONTENT_VERIFIER_DELEGATE_H_
