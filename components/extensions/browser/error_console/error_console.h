...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_ERROR_CONSOLE_ERROR_CONSOLE_H_
#define COMPONENTS_EXTENSIONS_BROWSER_ERROR_CONSOLE_ERROR_CONSOLE_H_
...
class ErrorConsole : public KeyedService,
 ...
 public:
  class Observer {
   public:
    ...
    virtual void OnErrorAdded(const extensions::ExtensionError* error);
  ...
  explicit ErrorConsole(content::BrowserContext* context);
  ...
  void SetReportingForExtension(const std::string& extension_id,
                                extensions::ExtensionError::Type type,
                                bool enabled);
  ...
  void ReportError(std::unique_ptr<extensions::ExtensionError> error);

  // Removes errors from the map according to the given |filter|.
  void RemoveErrors(const extensions::ErrorMap::Filter& filter);

  // Get a collection of weak pointers to all errors relating to the extension
  // with the given |extension_id|.
  const extensions::ErrorList& GetErrorsForExtension(const std::string& extension_id) const;
  ...
  void set_default_reporting_for_test(extensions::ExtensionError::Type type, bool enabled) {
    default_mask_ =
 ...
 private:
  ...
  void AddManifestErrorsForExtension(const extensions::Extension* extension);
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_ERROR_CONSOLE_ERROR_CONSOLE_H_
