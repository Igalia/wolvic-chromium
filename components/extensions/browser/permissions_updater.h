...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_PERMISSIONS_UPDATER_H__
#define COMPONENTS_EXTENSIONS_BROWSER_PERMISSIONS_UPDATER_H__
...
class PermissionsUpdater {
 public:
  ...
  void GrantOptionalPermissions(const extensions::Extension& extension,
  ...
  void GrantRuntimePermissions(const extensions::Extension& extension,
                               const extensions::PermissionSet& permissions,
                               base::OnceClosure completion_callback);
  ...
  // extension's optional permissions set through the permissions API.
  void RevokeOptionalPermissions(const extensions::Extension& extension,
                                 const extensions::PermissionSet& permissions,
  ...
  // host permissions feature.
  void RevokeRuntimePermissions(const extensions::Extension& extension,
                                const extensions::PermissionSet& permissions,
                                base::OnceClosure completion_callback);
  ...
  void RemovePermissionsUnsafe(const extensions::Extension* extension,
                               const extensions::PermissionSet& permissions);

  // Fetches the policy settings from the ExtensionManagement service and
  // applies them to the extension.
  void ApplyPolicyHostRestrictions(const extensions::Extension& extension);

  // Sets list of hosts |extension| may not interact with (overrides default).
  void SetPolicyHostRestrictions(const extensions::Extension* extension,
  ...
  void SetUsesDefaultHostRestrictions(const extensions::Extension* extension);
  ...
  void SetDefaultPolicyHostRestrictions(
      const extensions::URLPatternSet& default_runtime_blocked_hosts,
      const extensions::URLPatternSet& default_runtime_allowed_hosts);

  // Returns the set of revokable permissions.
  std::unique_ptr<const extensions::PermissionSet> GetRevokablePermissions(
  ...
  void AddPermissionsForTesting(const extensions::Extension& extension,
                                const extensions::PermissionSet& permissions);
 ...
 private:
  ...
  static void NotifyPermissionsUpdated(
      ...
      std::unique_ptr<const extensions::PermissionSet> changed,
      base::OnceClosure completion_callback);
  ...
  void SetPermissions(const extensions::Extension* extension,
  ...
  void AddPermissionsImpl(const extensions::Extension& extension,
                          const extensions::PermissionSet& active_permissions_to_add,
                          int prefs_permissions_store_mask,
                          const extensions::PermissionSet& permissions_to_add_to_prefs,
                          base::OnceClosure completion_callback);
  ...
  void RemovePermissionsImpl(
      ...
      std::unique_ptr<const extensions::PermissionSet> new_active_permissions,
      const extensions::PermissionSet& permissions_to_remove_from_prefs,
      int prefs_permissions_store_mask,
      base::OnceClosure completion_callback);
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_PERMISSIONS_UPDATER_H__
