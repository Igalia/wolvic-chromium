...
using extensions::CrxInstallErrorType;
...
CrxInstaller::CrxInstaller(base::WeakPtr<ExtensionService> service_weak,
                           std::unique_ptr<ExtensionInstallPrompt> client,
                           const WebstoreInstaller::Approval* approval)
    : browser_context_(service_weak->GetBrowserContext()),
      install_directory_(service_weak->install_directory()),
      install_source_(extensions::mojom::ManifestLocation::kInternal),
      approved_(false),
...
void CrxInstaller::UpdateExtensionFromUnpackedCrx(
  ...
  const Extension* extension = ExtensionRegistry::Get(service->GetBrowserContext())
...
void CrxInstaller::OnInstallChecksComplete(const PreloadCheck::Errors& errors) {
  ...
  if (!requirements_check_->GetErrorMessage().empty()) {
    ...
    install_flags_ |= extensions::kInstallFlagHasRequirementErrors;
  }
  ...
  if (errors.count(PreloadCheck::Error::kBlocklistedId) ||
    ...
    if (allow_silent_install_) {
      ...
      // to deal with it.
      if (errors.count(PreloadCheck::Error::kBlocklistedId))
        install_flags_ |= extensions::kInstallFlagIsBlocklistedForMalware;
    } else {
...
void CrxInstaller::ConfirmInstall() {
  ...
  current_version_ = base::Version(ExtensionPrefs::Get(service->GetBrowserContext())
...
void CrxInstaller::UpdateCreationFlagsAndCompleteInstall(
  ...
  if (ExtensionPrefs::Get(browser_context_)->AllowFileAccess(extension()->id()))
...
void CrxInstaller::NotifyCrxInstallBegin() {
  InstallTrackerFactory::GetForBrowserContext(browser_context_)->OnBeginCrxInstall(
      *this, expected_id_);
}
...
void CrxInstaller::ConfirmReEnable() {
  ...
  ExtensionPrefs* prefs = ExtensionPrefs::Get(service->GetBrowserContext());
  ...
  if (client_) {
    ...
    ExtensionInstallPrompt::PromptType type =
        ExtensionInstallPrompt::GetReEnablePromptTypeForExtension(
            service->GetBrowserContext(), extension());
    client_->ShowDialog(
        base::BindOnce(&CrxInstaller::OnInstallPromptDone, this), extension(),
...
base::SequencedTaskRunner* CrxInstaller::GetUnpackerTaskRunner() {
  if (!unpacker_task_runner_) {
    bool low_priority =
        (creation_flags_ & Extension::WAS_INSTALLED_BY_DEFAULT) &&
        !(creation_flags_ & Extension::WAS_INSTALLED_BY_OEM);
    unpacker_task_runner_ = extensions::GetOneShotFileTaskRunner(
