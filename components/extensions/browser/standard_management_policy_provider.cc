...
StandardManagementPolicyProvider::StandardManagementPolicyProvider(
    ...
    : browser_context_(context), settings_(settings) {}
...
bool StandardManagementPolicyProvider::MustRemainDisabled(
  ...
  if (!settings_->CheckMinimumVersion(extension, &required_version)) {
    if (reason)
      *reason = extensions::disable_reason::DISABLE_UPDATE_REQUIRED_BY_POLICY;
    if (error) {
      *error = l10n_util::GetStringFUTF16(
  ...
  if (!settings_->IsAllowedByUnpublishedAvailabilityPolicy(extension)) {
    if (reason) {
      *reason = extensions::disable_reason::DISABLE_PUBLISHED_IN_STORE_REQUIRED_BY_POLICY;
    }
    if (error) {
      *error = l10n_util::GetStringFUTF16(
...
}  // namespace namespace components_extensions {
