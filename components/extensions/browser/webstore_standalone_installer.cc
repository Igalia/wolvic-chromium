...
#include "components/extensions/browser/webstore_standalone_installer.h"
...
#include "components/extensions/browser/webstore_data_fetcher.h"
...
// TODO(mshin): Enable the below code after support Profile
// namespace {
...
WebstoreStandaloneInstaller::WebstoreStandaloneInstaller(
    ...
    Callback callback)
    : id_(webstore_item_id), callback_(std::move(callback))
...
bool WebstoreStandaloneInstaller::EnsureUniqueInstall(
    ...
    std::string* error) {
  InstallTracker* tracker = InstallTracker::Get(browser_context_);
  DCHECK(tracker);
...
WebstoreStandaloneInstaller::CreateApproval() const {
  std::unique_ptr<WebstoreInstaller::Approval> approval(
      WebstoreInstaller::Approval::CreateWithNoInstallPrompt(
          browser_context_, id_, manifest_->Clone(), true));
...
void WebstoreStandaloneInstaller::OnInstallPromptDone(
  ...
  ExtensionRegistry* extension_registry = ExtensionRegistry::Get(browser_context_);
  ...
  if (installed_extension) {
    ...
    if (blocklist_prefs::IsExtensionBlocklisted(
            id_, ExtensionPrefs::Get(browser_context_))) {
...
void WebstoreStandaloneInstaller::OnWebstoreResponseParseSuccess(
  ...
  helper->Start(browser_context_->GetDefaultStoragePartition()
                    ->GetURLLoaderFactoryForBrowserProcess()
                    .get());
}
...
// TODO(mshin): Enable the below code after support Profile
// void WebstoreStandaloneInstaller::OnProfileWillBeDestroyed(BrowserContext* browser_context) {
//   DCHECK(profile == browser_context_);
