...
ExtensionManagement::InstallationMode ExtensionManagement::GetInstallationMode(
  ...
  const std::string* update_url =
      extension->manifest()->FindStringPath(extensions::manifest_keys::kUpdateURL);
...
void ExtensionManagement::Refresh() {
  ...
  const base::Value::List* allowed_list_pref =
      LoadListPreference(extensions::pref_names::kInstallAllowList, true);
  ...
  const base::Value::List* denied_list_pref =
      LoadListPreference(extensions::pref_names::kInstallDenyList, false);
  const base::Value::Dict* forced_list_pref =
      LoadDictPreference(extensions::pref_names::kInstallForceList, true);
  const base::Value::List* install_sources_pref =
      LoadListPreference(extensions::pref_names::kAllowedInstallSites, true);
  ...
  const base::Value* unpublished_availability_pref =
      LoadPreference(extensions::pref_names::kExtensionUnpublishedAvailability,
                     /*force_managed=*/true, base::Value::Type::INTEGER);
...
base::Value::Dict ExtensionManagement::GetInstallListByMode(
  ...
  // TODO(mshin): Enable the below code after migrating ExternalPolicyLoader
...
ExtensionManagementFactory::BuildServiceInstanceForBrowserContext(
  ...
  return std::make_unique<ExtensionManagement>(context);
}

void ExtensionManagementFactory::RegisterProfilePrefs(
    user_prefs::PrefRegistrySyncable* user_prefs) {
  user_prefs->RegisterDictionaryPref(extensions::pref_names::kExtensionManagement);
}
