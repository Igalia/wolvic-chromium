...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_CRX_INSTALLER_H_
#define COMPONENTS_EXTENSIONS_BROWSER_CRX_INSTALLER_H_
...
class CrxInstaller : public extensions::SandboxedUnpackerClient {
 public:
  ...
  using InstallerResultCallback = extensions::ExtensionSystem::InstallUpdateCallback;
  ...
  virtual void InstallCrxFile(const extensions::CRXFileInfo& source_file);

  // Install the unpacked crx in |unpacked_dir|.
  // If |delete_source_| is true, |unpacked_dir| will be removed at the end of
  // the installation.
  void InstallUnpackedCrx(const extensions::ExtensionId& extension_id,
  ...
  // If |delete_source_| is true, |unpacked_dir| will be removed at the end of
  // the update.
  void UpdateExtensionFromUnpackedCrx(const extensions::ExtensionId& extension_id,
                                      const std::string& public_key,
                                      const base::FilePath& unpacked_dir);

  void OnInstallPromptDone(ExtensionInstallPrompt::DoneCallbackPayload payload);

  void InitializeCreationFlagsForUpdate(const extensions::Extension* extension,
  ...
  extensions::mojom::ManifestLocation install_source() const { return install_source_; }
  void set_install_source(extensions::mojom::ManifestLocation source) {
    install_source_ = source;
  }

  const extensions::ExtensionId& expected_id() const { return expected_id_; }
  void set_expected_id(const extensions::ExtensionId& val) { expected_id_ = val; }
  ...
  void set_install_immediately(bool val) {
    set_install_flag(extensions::kInstallFlagInstallImmediately, val);
  }
  void set_do_not_sync(bool val) {
    set_install_flag(extensions::kInstallFlagDoNotSync, val);
  }
  void set_bypassed_safebrowsing_friction_for_testing(bool val) {
    set_install_flag(extensions::kInstallFlagBypassedSafeBrowsingFriction, val);
  }
 ...
 protected:
  ...
  void RunInstallerCallbacks(const std::optional<extensions::CrxInstallError>& error);
 ...
 private:
  ...
  std::optional<extensions::CrxInstallError> CheckExpectations(const extensions::Extension* extension);

  // Called after OnUnpackSuccess as a last check to see whether the install
  // should complete.
  std::optional<extensions::CrxInstallError> AllowInstall(const extensions::Extension* extension);
  ...
  void ShouldComputeHashesOnUI(scoped_refptr<const extensions::Extension> extension,
                               base::OnceCallback<void(bool)> callback);

  // To provide content verifier key to the unpacker.
  void GetContentVerifierKeyOnUI(
      base::OnceCallback<void(extensions::ContentVerifierKey)> callback);

  // SandboxedUnpackerClient
  void GetContentVerifierKey(
      base::OnceCallback<void(extensions::ContentVerifierKey)> callback) override;
  ...
  void OnStageChanged(extensions::InstallationStage stage) override;
  ...
  void OnInstallChecksComplete(const extensions::PreloadCheck::Errors& errors);
  ...
  // Result reporting.
  void ReportFailureFromSharedFileThread(const extensions::CrxInstallError& error);
  void ReportFailureFromUIThread(const extensions::CrxInstallError& error);
  void ReportSuccessFromSharedFileThread();
  void ReportSuccessFromUIThread();
  // Always report from the UI thread.
  void ReportInstallationStage(extensions::InstallationStage stage);
  void NotifyCrxInstallBegin();
  void NotifyCrxInstallComplete(const std::optional<extensions::CrxInstallError>& error);
  ...
  virtual void OnUnpackSuccessOnSharedFileThread(
      ...
      extensions::declarative_net_request::RulesetInstallPrefs ruleset_install_prefs);
  ...
  extensions::mojom::ManifestLocation install_source_;
  ...
  extensions::ExtensionId expected_id_;
  ...
  extensions::declarative_net_request::RulesetInstallPrefs ruleset_install_prefs_;

  // Checks that may run before installing the extension.
  std::unique_ptr<extensions::PreloadCheck> policy_check_;
  std::unique_ptr<extensions::PreloadCheck> requirements_check_;
  std::unique_ptr<extensions::PreloadCheck> blocklist_check_;

  // Runs the above checks.
  std::unique_ptr<extensions::PreloadCheckGroup> check_group_;
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_CRX_INSTALLER_H_
