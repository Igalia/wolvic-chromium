...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_CHROME_TEST_EXTENSION_LOADER_H_
#define COMPONENTS_EXTENSIONS_BROWSER_CHROME_TEST_EXTENSION_LOADER_H_
...
class ChromeTestExtensionLoader {
 public:
  ...
  // unpacked extensions.
  scoped_refptr<const extensions::Extension> LoadExtension(const base::FilePath& file_path);
  ...
  void LoadUnpackedExtensionAsync(
      ...
      base::OnceCallback<void(const extensions::Extension*)> callback);
  ...
  void add_creation_flag(extensions::Extension::InitFromValueFlags flag) {
    creation_flags_ |= flag;
  }
  void set_creation_flags(int flags) { creation_flags_ = flags; }
  void set_location(extensions::mojom::ManifestLocation location) { location_ = location; }
 ...
 private:
  ...
  scoped_refptr<const extensions::Extension> LoadCrx(const base::FilePath& crx_path);
  ...
  void CheckPermissions(const extensions::Extension* extension);

  // Verifies that the permissions of the loaded extension are correct.
  // Returns false if they are not.
  bool VerifyPermissions(const extensions::Extension* extension);

  // Checks for any install warnings associated with the extension.
  bool CheckInstallWarnings(const extensions::Extension& extension);

  // Waits for the extension to finish setting up.
  bool WaitForExtensionReady(const extensions::Extension& extension);
  ...
  int creation_flags_ = extensions::Extension::NO_FLAGS;

  // The install location of the added extension. Not valid for unpacked
  // extensions.
  extensions::mojom::ManifestLocation location_ = extensions::mojom::ManifestLocation::kInternal;
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_CHROME_TEST_EXTENSION_LOADER_H_
