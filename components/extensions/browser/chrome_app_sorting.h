...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_CHROME_APP_SORTING_H_
#define COMPONENTS_EXTENSIONS_BROWSER_CHROME_APP_SORTING_H_
...
// TODO(mshin): Enable below codes after supporting WebApps
// namespace web_app {
// class WebApp;
...
class ChromeAppSorting : public extensions::AppSorting,
 ...
 public:
  ...
  bool GetDefaultOrdinals(const extensions::ExtensionId& extension_id,
                          syncer::StringOrdinal* page_ordinal,
                          syncer::StringOrdinal* app_launch_ordinal) override;
  void OnExtensionMoved(const extensions::ExtensionId& moved_extension_id,
                        const extensions::ExtensionId& predecessor_extension_id,
                        const extensions::ExtensionId& successor_extension_id) override;
  ...
  syncer::StringOrdinal GetPageOrdinal(
      const extensions::ExtensionId& extension_id) const override;
  void SetPageOrdinal(const extensions::ExtensionId& extension_id,
                      const syncer::StringOrdinal& new_page_ordinal) override;
  void ClearOrdinals(const extensions::ExtensionId& extension_id) override;
  int PageStringOrdinalAsInteger(
      const syncer::StringOrdinal& page_ordinal) const override;
  syncer::StringOrdinal PageIntegerAsStringOrdinal(size_t page_index) override;
  void SetExtensionVisible(const extensions::ExtensionId& extension_id,
 ...
 private:
  ...
  using AppOrdinalsMap = std::map<extensions::ExtensionId, AppOrdinals>;
  ...
  void InitializePageOrdinalMap(
      const std::vector<extensions::ExtensionId>& extension_or_app_ids);

  // Migrates the app launcher and page index values.
  void MigrateAppIndex(const extensions::ExtensionIdList& extension_ids);
  ...
  void AddOrdinalMapping(const extensions::ExtensionId& extension_id,
                         const syncer::StringOrdinal& page_ordinal,
  ...
  // StringOrdinals.
  void RemoveOrdinalMapping(const extensions::ExtensionId& extension_id,
                            const syncer::StringOrdinal& page_ordinal,
                            const syncer::StringOrdinal& app_launch_ordinal);

  // Syncs the extension if needed. It is an error to call this if the
  // extension is not an application.
  void SyncIfNeeded(const extensions::ExtensionId& extension_id);
  ...
  std::set<extensions::ExtensionId> ntp_hidden_extensions_;
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_CHROME_APP_SORTING_H_
