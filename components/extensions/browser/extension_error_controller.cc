...
#include "components/extensions/browser/extension_error_ui_default.h"
...
void ExtensionErrorController::ShowErrorIfNeeded() {
  // TODO(mshin): Remove the below code after support GlobalError UI
  if (!blocklisted_extensions_.empty()) {
    LOG(ERROR) << "Blocked extensions list=>";
    for (const auto& extension : blocklisted_extensions_) {
      LOG(ERROR) << "\tExtension: " << extension->name();
    }
  }
