...
ExtensionPrefs* TestExtensionPrefs::prefs() {
  return ExtensionPrefs::Get(&browser_context_);
}

TestBrowserContext* TestExtensionPrefs::browser_context() {
...
void TestExtensionPrefs::ResetPrefRegistry() {
  ...
  TestExtensionEnvironment::RegisterProfilePrefs(pref_registry_.get());
}
...
void TestExtensionPrefs::RecreateExtensionPrefs() {
  ...
  std::unique_ptr<ExtensionPrefs> prefs(ExtensionPrefs::Create(
      &browser_context_, new_pref_service.get(), temp_dir_.GetPath(),
      ...
      clock_.get()));
  ExtensionPrefsFactory::GetInstance()->SetInstanceForTesting(&browser_context_,
                                                              std::move(prefs));
  ...
  static_cast<TestExtensionSystem*>(ExtensionSystem::Get(&browser_context_))
      ->RecreateAppSorting();
}
...
ChromeAppSorting* TestExtensionPrefs::app_sorting() {
  return static_cast<ChromeAppSorting*>(
      ExtensionSystem::Get(&browser_context_)->app_sorting());
}
