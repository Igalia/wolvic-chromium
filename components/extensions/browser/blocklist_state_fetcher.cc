...
BlocklistStateFetcher::BlocklistStateFetcher() {
  url_loader_factory_ =
      static_cast<ChromeExtensionsBrowserClient*>(
          ExtensionsBrowserClient::Get())->GetSharedUrlLoaderFactory();
}
  ...
  if (!safe_browsing_config_) {
    ...
    // } else {
      base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
          FROM_HERE, base::BindOnce(std::move(callback), extensions::BLOCKLISTED_UNKNOWN));
      return;
    // }
  }
...
void BlocklistStateFetcher::OnURLLoaderCompleteInternal(
  ...
  if (net_error == net::OK && response_code == 200) {
    ...
    if (response.ParseFromString(response_body)) {
      ...
      state = extensions::BLOCKLISTED_UNKNOWN;
    }
  } else {
    if (net_error != net::OK) {
