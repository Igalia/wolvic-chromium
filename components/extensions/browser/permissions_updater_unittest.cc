...
#include "components/extensions/common/extension_test_util.h"
...
using extensions::PermissionsManagerWaiter;
...
TEST_F(PermissionsUpdaterTest, GrantAndRevokeOptionalPermissions) {
...

  {
    ...
    PermissionsManagerWaiter waiter(PermissionsManager::Get(browser_context()));
...
TEST_F(PermissionsUpdaterTest, RevokingPermissions) {
...

  {
    ...
    permissions_test_util::GrantOptionalPermissionsAndWaitForCompletion(
        browser_context(), *extension, *api_permission_set(APIPermissionID::kCookie));
    ...
    // Repeat with "tabs".
    permissions_test_util::GrantOptionalPermissionsAndWaitForCompletion(
        browser_context(), *extension, *api_permission_set(APIPermissionID::kTab));
...

  {
    ...
    PermissionsData::SetDefaultPolicyHostRestrictions(
        util::GetBrowserContextId(browser_context()), default_policy_blocked_hosts,
...
TEST_F(PermissionsUpdaterTest,
  ...
  // granted permissions.
  permissions_test_util::GrantOptionalPermissionsAndWaitForCompletion(
      browser_context(), *extension, optional_permissions);
...
TEST_F(PermissionsUpdaterTest,
  ...
  permissions_test_util::GrantRuntimePermissionsAndWaitForCompletion(
      browser_context(), *extension, runtime_granted_permissions);
...
TEST_F(PermissionsUpdaterTest, RevokingPermissionsWithRuntimeHostPermissions) {
  ...
  for (const auto& test_case : test_cases) {
    ...
    // should be revokable.
    permissions_test_util::GrantRuntimePermissionsAndWaitForCompletion(
        browser_context(), *extension, permission_set);
    EXPECT_TRUE(extension->permissions_data()
                    ->active_permissions()
                    .HasExplicitAccessToOrigin(kOrigin));
    EXPECT_TRUE(updater.GetRevokablePermissions(extension.get())
...
TEST_F(PermissionsUpdaterTest, GrantingBroadRuntimePermissions) {
  ...
  permissions_test_util::GrantRuntimePermissionsAndWaitForCompletion(
      browser_context(), *extension, runtime_permissions);
...
TEST_F(PermissionsUpdaterTest,
  ...
  scoped_refptr<const Extension> extension =
      ChromeTestExtensionLoader(browser_context()).LoadExtension(
