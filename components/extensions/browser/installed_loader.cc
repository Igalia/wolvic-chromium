...
bool IsManifestCorrupt(const base::Value::Dict& manifest) {
  ...
  return manifest.contains(extensions::manifest_keys::kBackgroundPage) &&
         manifest.contains(extensions::manifest_keys::kBackgroundScripts);
}
...
void RecordDisableReasons(int reasons) {
  ...
  if (reasons == extensions::disable_reason::DISABLE_NONE) {
    RecordDisbleReasonHistogram(extensions::disable_reason::DISABLE_NONE);
    return;
  }
  for (int reason = 1; reason < extensions::disable_reason::DISABLE_REASON_LAST;
       reason <<= 1) {
    if (reasons & reason)
...
HostPermissionsAccess GetHostPermissionAccessLevelForExtension(
  ...
  if (!extensions::util::CanWithholdPermissionsFromExtension(extension))
  ...
  if (active_hosts_size == 1) {
    ...
    if (single_pattern.scheme() != content::kChromeUIScheme ||
        single_pattern.host() != kChromeUIFaviconHost)
      return HostPermissionsAccess::kOnSpecificSites;
  }
...
bool ShouldCollectDevModeDataForLocation(extensions::mojom::ManifestLocation location) {
  return location == extensions::mojom::ManifestLocation::kExternalPref ||
         location == extensions::mojom::ManifestLocation::kExternalPrefDownload ||
         location == extensions::mojom::ManifestLocation::kExternalRegistry ||
...
InstalledLoader::InstalledLoader(ExtensionService* extension_service)
    : extension_service_(extension_service),
      extension_registry_(ExtensionRegistry::Get(extension_service->GetBrowserContext())),
      extension_prefs_(ExtensionPrefs::Get(extension_service->GetBrowserContext())) {}
...
void InstalledLoader::Load(const ExtensionInfo& info, bool write_to_prefs) {
  ...
  if (info.extension_manifest) {
    ...
    error = extensions::manifest_errors::kManifestUnreadable;
  }
  ...
  if (extension.get() && !Manifest::IsUnpackedLocation(extension->location()) &&
    ...
    error = extensions::manifest_errors::kCannotChangeExtensionID;
    extension = nullptr;
  }

  if (!extension.get()) {
    LoadErrorReporter::GetInstance()->ReportLoadError(
        info.extension_path, error, extension_service_->GetBrowserContext(),
        false);  // Be quiet.
    return;
  }

  const ManagementPolicy* policy =
      ExtensionSystem::Get(extension_service_->GetBrowserContext())->management_policy();
...
void InstalledLoader::LoadAllExtensions() {
  LoadAllExtensions(extension_service_->GetBrowserContext());
}

void InstalledLoader::LoadAllExtensions(content::BrowserContext* context) {
  ...
  for (auto& info : extensions_info) {
    ...
    if (info.extension_location == extensions::mojom::ManifestLocation::kCommandLine) {
      continue;
    }
    ...
    if (ShouldReloadExtensionManifest(info) != NOT_NEEDED) {
      ...
      std::string error;
      scoped_refptr<const Extension> extension(
          extensions::file_util::LoadExtension(info.extension_path, info.extension_location,
                                   GetCreationFlags(&info), &error));
  ...
  RecordExtensionsMetrics(context, is_user_profile);
...
void InstalledLoader::RecordExtensionsMetricsForTesting() {
  RecordExtensionsMetrics(/*context=*/extension_service_->GetBrowserContext(),
...
void InstalledLoader::RecordExtensionsMetrics(content::BrowserContext* context,
  ...
  for (ExtensionSet::const_iterator iter = extensions.begin();
    ...
    if (is_user_profile) {
      bool dev_mode_enabled =
          extensions::GetCurrentDeveloperMode(
    ...
    if (extension->permissions_data()->HasAPIPermission(
            extensions::mojom::APIPermissionID::kWebRequest)) {
      web_request_count++;
    }
    ...
    // unpacked extensions.
    // TODO(mshin): Enable the below code after migrating chrome_manifest_url_handlers
    ...
    // TODO(mshin): Enable the below code after migrating SettingsOverrides
    ...
    if (type == Manifest::TYPE_EXTENSION) {
      ...
      if (GetBackgroundPageType(extension) == EVENT_PAGE) {
        ...
        if (!EventRouter::Get(extension_service_->GetBrowserContext())
    ...
    if (extension->manifest()->FindKey(extensions::manifest_keys::kPageAction))
      ++page_action_count;
    else if (extension->manifest()->FindKey(extensions::manifest_keys::kBrowserAction))
    ...
    if (extensions::ui_util::ShouldDisplayInExtensionSettings(*extension) &&
      ...
      if (extension->wants_file_access()) {
        if (extensions::util::AllowFileAccess(extension->id(), context))
    ...
    PermissionsManager* permissions_manager = PermissionsManager::Get(context);
  ...
  for (const scoped_refptr<const Extension>& disabled_extension :
       disabled_extensions) {
    extensions::mojom::ManifestLocation location = disabled_extension->location();
  ...
  base::UmaHistogramCounts100(
      ...
      extension_prefs_->GetPrefAsInteger(extensions::kCorruptedDisableCount));
