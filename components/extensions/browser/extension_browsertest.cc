...
#include "components/extensions/browser/extension_browsertest.h"
...
namespace service_worker_test_utils = extensions::service_worker_test_utils;
...
using extensions::ExtensionCacheFake;
...
using extensions::ScopedIgnoreContentVerifierForTest;
using extensions::ScopedTestDialogAutoConfirm;
using extensions::SetExtensionProtocolTestHandler;
...
BrowserContext* ExtensionBrowserTest::browser_context() {
  DCHECK(browser_context_);
  return browser_context_.get();
}

WebContents* ExtensionBrowserTest::web_contents() {
...
base::FilePath ExtensionBrowserTest::GetTestResourcesParentDir() {
  ...
  base::PathService::Get(DIR_TEST_DATA, &test_root_path);
...
void ExtensionBrowserTest::PreRunTestOnMainThread() {
  ...
  BrowserContext* initial_browser_context =
      shell()->web_contents()->GetBrowserContext();
  ASSERT_TRUE(initial_browser_context);
  browser_context_ = initial_browser_context;

  local_state_ = shell_prefs::CreateLocalState(browser_context_->GetPath());
  user_pref_service_ = shell_prefs::CreateUserPrefService(browser_context_.get());
  ...
  ExtensionSystem::Get(browser_context_.get())->InitForRegularProfile(true);
...
void ExtensionBrowserTest::SetUpCommandLine(base::CommandLine* command_line) {
  base::PathService::Get(DIR_TEST_DATA, &test_data_dir_);
...
//       ui::kScaleFactorNone);
  ...
  observer_ =
      std::make_unique<ChromeExtensionTestNotificationObserver>(browser_context());
  ...
  // TODO(mshin): Support theme
  // content::URLDataSource::Add(browser_context(),
  //                             std::make_unique<ThemeSource>(browser_context()));
...
EventRouterForwarder* ExtensionBrowserTest::extension_event_router_forwarder() {
  return extension_event_router_forwarder_.get();  
}

network::mojom::NetworkContext* ExtensionBrowserTest::GetNetworkContext() {
  return browser_context()->GetDefaultStoragePartition()->GetNetworkContext();
}

bool ExtensionBrowserTest::IsShuttingDown() {
  return false;
}

std::string ExtensionBrowserTest::GetApplicationLocale() {
  return "en-US";
}

std::vector<BrowserContext*> ExtensionBrowserTest::GetAllBrowserContexts() {
  return { browser_context_.get() };
}
...
PrefService*
ExtensionBrowserTest::GetPrefServiceForContext(BrowserContext* context) {
...
const Extension* ExtensionBrowserTest::LoadExtension(
  ...
  if (options.wait_for_registration_stored) {
    registration_observer =
        std::make_unique<TestRegistrationObserver>(browser_context_);
  }
...
const Extension* ExtensionBrowserTest::LoadExtensionAsComponent(
    const base::FilePath& path) {
  return LoadExtensionAsComponentWithManifest(path, extensions::kManifestFilename);
}
...
const Extension* ExtensionBrowserTest::UpdateExtensionWaitForIdle(
  ...
  return InstallOrUpdateExtension(id, path, INSTALL_UI_TYPE_NONE,
                                  ...
                                  ManifestLocation::kInternal, browser_context(),
...
const Extension* ExtensionBrowserTest::InstallExtensionFromWebstore(
  ...
  return InstallOrUpdateExtension(
      ...
      std::move(expected_change), ManifestLocation::kInternal, browser_context(),
...
const Extension* ExtensionBrowserTest::InstallOrUpdateExtension(
  ...
  return InstallOrUpdateExtension(id, path, ui_type, std::move(expected_change),
                                  install_source, browser_context(),
                                  Extension::NO_FLAGS, true, false);
}
...
base::Value ExtensionBrowserTest::ExecuteScriptInBackgroundPage(
  ...
  return extensions::browsertest_util::ExecuteScriptInBackgroundPage(
      browser_context(), extension_id, script, script_user_activation);
}
...
content::ServiceWorkerContext* ExtensionBrowserTest::GetServiceWorkerContext() {
  return GetServiceWorkerContext(browser_context());
}
...
bool ExtensionBrowserTest::ModifyExtensionIfNeeded(
  ...
  base::FilePath manifest_path = extension_root.Append(extensions::kManifestFilename);
