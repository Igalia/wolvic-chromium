...
ExtensionInstallStatus GetWebstoreExtensionInstallStatus(
  ...
  return GetWebstoreExtensionInstallStatus(
      extension_id, browser_context, Manifest::Type::TYPE_UNKNOWN, PermissionSet());
}
...
ExtensionInstallStatus GetWebstoreExtensionInstallStatus(
  ...
  if (ExtensionPrefs::Get(browser_context)->HasDisableReason(
  ...
  if (!prefs->GetBoolean(prefs::kCloudExtensionRequestEnabled))
    return kBlockedByPolicy;
  ...
  if (prefs->GetDict(prefs::kCloudExtensionRequestIds).Find(extension_id)) {
