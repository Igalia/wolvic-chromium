...
#include "components/extensions/browser/scoped_active_install.h"
...
using components_extensions::ActiveInstallData;
using components_extensions::ExtensionInstallStatus;
using components_extensions::ExtensionManagementFactory;
using components_extensions::GetWebstoreExtensionInstallStatus;
using components_extensions::InstallTracker;
using components_extensions::ScopedActiveInstall;
using components_extensions::WebstoreInstaller;
using components_extensions::WebstoreInstallHelper;
...
class PendingApprovals/* : public ProfileObserver*/ {
 ...
 private:
  ...
  // void OnProfileWillBeDestroyed(BrowserContext* browser_context) override {
  //   std::erase_if(approvals_, [browser_context](const auto& approval) {
  //     return approval->browser_context == browser_context;
  //   });
  //   observation_.RemoveObservation(browser_context);
  // }

  void MaybeAddObservation(BrowserContext* browser_context) {
    // TODO(mshin): Enable the below code after supporting Profile
    // if (!observation_.IsObservingSource(browser_context))
    //   observation_.AddObservation(browser_context);
  }

  // Remove observation if there are no pending approvals
  // for the BrowserContext.
  void MaybeRemoveObservation(BrowserContext* browser_context) {
    // TODO(mshin): Enable the below code after supporting Profile
...
void PendingApprovals::PushApproval(
  ...
  MaybeAddObservation(approval->browser_context);
  ...
  for (auto iter = approvals_.begin(); iter != approvals_.end(); ++iter) {
    if (iter->get()->extension_id == id &&
        ExtensionsBrowserClient::Get()->IsSameContext(
            browser_context, iter->get()->browser_context)) {
      std::unique_ptr<WebstoreInstaller::Approval> approval = std::move(*iter);
      approvals_.erase(iter);
      MaybeRemoveObservation(approval->browser_context);
      return approval;
    }
  }
  return nullptr;
...
// TODO(mshin): Support SafeBrowsing
...
std::string GetWebstoreLogin(BrowserContext* browser_context) {
  auto* prefs =
      ExtensionsBrowserClient::Get()->GetPrefServiceForContext(browser_context);
  DCHECK(prefs);
...
void SetWebstoreLogin(BrowserContext* browser_context, const std::string& login) {
  auto* prefs =
      ExtensionsBrowserClient::Get()->GetPrefServiceForContext(browser_context);
  DCHECK(prefs);
  prefs->SetString(kWebstoreLogin, login);
}
...
ExtensionInstallStatus AddExtensionToPendingList(
  ...
  if (status != components_extensions::kCanRequest &&
      status != components_extensions::kInstallable &&
      status != components_extensions::kBlockedByPolicy &&
      status != components_extensions::kForceInstalled) {
    return status;
  }
...
#if DCHECK_IS_ON()
  if (status == components_extensions::kCanRequest)
    DCHECK_EQ(components_extensions::kRequestPending, new_status);
  else
...
WebstorePrivateApi::PopApprovalForTesting(BrowserContext* browser_context,
...
void WebstorePrivateBeginInstallWithManifest3Function::OnWebstoreParseSuccess(
  ...
  ExtensionInstallStatus install_status = GetWebstoreExtensionInstallStatus(
      id, browser_context(), dummy_extension_->manifest()->type(),
      PermissionsParser::GetRequiredPermissions(dummy_extension_.get()),
      dummy_extension_->manifest_version());
  if (install_status == components_extensions::kBlockedByPolicy) {
  ...
  if (install_status == components_extensions::kCanRequest ||
      install_status == components_extensions::kRequestPending) {
    ...
    install_prompt_->ShowDialog(
        ...
        std::make_unique<ExtensionInstallPrompt::Prompt>(
            install_status == components_extensions::kCanRequest
    ...
    if (ShouldShowFrictionDialog(browser_context())) {
...
void WebstorePrivateBeginInstallWithManifest3Function::OnRequestPromptDone(
  ...
  switch (payload.result) {
    case ExtensionInstallPrompt::Result::ACCEPTED:
      AddExtensionToPendingList(details().id, browser_context(), payload.justification);
      break;
    case ExtensionInstallPrompt::Result::USER_CANCELED:
    case ExtensionInstallPrompt::Result::ABORTED:
      break;
...
void WebstorePrivateBeginInstallWithManifest3Function::HandleInstallProceed(
  ...
  std::unique_ptr<WebstoreInstaller::Approval> approval(
      WebstoreInstaller::Approval::CreateWithNoInstallPrompt(
          browser_context(), details().id, std::move(*parsed_manifest_), false));
...
bool WebstorePrivateBeginInstallWithManifest3Function::ShouldShowFrictionDialog(
  ...
  // Only show friction if the allowlist warnings are enabled for the browser_context.
  auto* extension_system = ExtensionSystem::Get(browser_context);
...
void WebstorePrivateBeginInstallWithManifest3Function::
  ...
  // TODO(mshin): Enable the below code after migrating ui/view/extensions/*
...
WebstorePrivateCompleteInstallFunction::Run() {
  ...
  // TODO(mshin): Enable the below code after supporting Profile
  if (/*browser_context()->IsGuestSession() || */browser_context()->IsOffTheRecord()) {
  ...
  approval_ =
      g_pending_approvals.Get().PopApproval(browser_context(), params->expected_id);
  if (!approval_) {
    return RespondNow(
...
ExtensionFunction::ResponseAction WebstorePrivateGetStoreLoginFunction::Run() {
  return RespondNow(ArgumentList(GetStoreLogin::Results::Create(
      GetWebstoreLogin(browser_context()))));
}
...
WebstorePrivateIsInIncognitoModeFunction::Run() {
  return RespondNow(ArgumentList(IsInIncognitoMode::Results::Create(
      browser_context() != ExtensionsBrowserClient::Get()->GetOriginalContext(
            browser_context()))));
}
...
void WebstorePrivateGetExtensionStatusFunction::OnManifestParsed(
  ...
  // TODO(mshin): Support Profile
  // if (!browser_context->IsValidProfile(profile)) {
  ...
  ExtensionInstallStatus status = GetWebstoreExtensionInstallStatus(
      extension_id, browser_context(), dummy_extension->GetType(),
