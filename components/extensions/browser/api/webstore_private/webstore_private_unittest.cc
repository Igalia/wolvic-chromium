...
// Verifies that the extension request pending list in |browser_context| matches the
...
void VerifyPendingList(const std::map<ExtensionId, ExtensionRequestData>&
  ...
  const base::Value::Dict& actual_pending_requests =
      prefs->GetDict(prefs::kCloudExtensionRequestIds);
...
void SetExtensionSettings(const std::string& settings_string,
  ...
  env->GetTestingPrefService()->SetManagedPref(
...
class WebstorePrivateGetExtensionStatusTest
 ...
 public:
  void SetUp() override {
    ...
    env_ = std::make_unique<TestExtensionEnvironment>(
...
TEST_F(WebstorePrivateGetExtensionStatusTest, ExtensionBlockedByManifestType) {
  SetExtensionSettings(kBlockedManifestTypeExtensionSettings, env());
  auto function =
      base::MakeRefCounted<WebstorePrivateGetExtensionStatusFunction>();
...
TEST_F(WebstorePrivateGetExtensionStatusTest, ExtensionBlockedByPermission) {
  SetExtensionSettings(kBlockedDownloadsPermissionsExtensionSettings, env());
  auto function =
      base::MakeRefCounted<WebstorePrivateGetExtensionStatusFunction>();
  std::optional<base::Value> response = RunFunctionAndReturnValue(
      function.get(), GenerateArgs(kExtensionId, kExtensionManifest));
...
TEST_F(WebstorePrivateGetExtensionStatusTest,
  ...
  SetExtensionSettings(kBlockedAudioPermissionsExtensionSettings, env());
  auto function =
      base::MakeRefCounted<WebstorePrivateGetExtensionStatusFunction>();
  std::optional<base::Value> response = RunFunctionAndReturnValue(
      function.get(), GenerateArgs(kExtensionId, kExtensionManifest));
 ...
 public:
  ...
  void SetUp() override {
    ...
    ManagementAPI::GetFactoryInstance()->SetTestingFactory(
        browser_context(), base::BindRepeating(&BuildManagementApi));
...
TEST_F(WebstorePrivateBeginInstallWithManifest3Test,
  ...
  VerifyPendingList({}, browser_context());
...
// TODO(mshin): Enable the below tests after migrating ui/view/extensions/*
TEST_P(WebstorePrivateBeginInstallWithManifest3FrictionDialogTest,
       DISABLED_FrictionDialogTests) {
  ...
  if (test_case.esb_user) {
    ...
    safe_browsing::SetSafeBrowsingState(
        prefs, safe_browsing::SafeBrowsingState::ENHANCED_PROTECTION);
  }
  ...
  std::unique_ptr<WebstoreInstaller::Approval> approval =
      WebstorePrivateApi::PopApprovalForTesting(browser_context(), kExtensionId);
  if (test_case.dialog_action == ScopedTestDialogAutoConfirm::ACCEPT) {
