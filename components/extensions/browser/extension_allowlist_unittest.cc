...
#include "components/extensions/browser/extension_management_test_util.h"
...
TEST_F(ExtensionAllowlistUnitTest, AllowlistEnforcement) {
  ...
  EXPECT_EQ(extensions::ALLOWLIST_NOT_ALLOWLISTED,
            allowlist()->GetExtensionAllowlistState(kExtensionId1));
...
TEST_F(ExtensionAllowlistUnitTest, EnforcementOnInit) {
  ...
  // previously disabled for this browser_context.
...
TEST_F(ExtensionAllowlistUnitTest, EnhancedProtectionSettingChange) {
  ...
  // `extensions::ALLOWLIST_NOT_ALLOWLISTED` state.
  safe_browsing::SetSafeBrowsingState(
      ExtensionsBrowserClient::Get()->GetPrefServiceForContext(browser_context()),
      safe_browsing::SafeBrowsingState::ENHANCED_PROTECTION);
  EXPECT_EQ(extensions::ALLOWLIST_NOT_ALLOWLISTED,
...
TEST_F(ExtensionAllowlistUnitTest, ExtensionsNotAllowlistedThenBlocklisted) {
  ...
  test_blocklist.SetBlocklistState(kExtensionId1, extensions::BLOCKLISTED_MALWARE, true);
  test_blocklist.SetBlocklistState(kExtensionId2,
                                   extensions::BLOCKLISTED_POTENTIALLY_UNWANTED, true);
  ...
  test_blocklist.SetBlocklistState(kExtensionId1, extensions::NOT_BLOCKLISTED, true);
  test_blocklist.SetBlocklistState(kExtensionId2, extensions::NOT_BLOCKLISTED, true);
  base::RunLoop().RunUntilIdle();
...
TEST_F(ExtensionAllowlistUnitTest, MissingAttributeAreIgnored) {
  ...
  allowlist()->SetExtensionAllowlistState(kExtensionId1, extensions::ALLOWLIST_ALLOWLISTED);
  allowlist()->SetExtensionAllowlistState(kExtensionId2,
                                          extensions::ALLOWLIST_NOT_ALLOWLISTED);
...
TEST_F(ExtensionAllowlistUnitTest, AcknowledgeNeededOnEnforcement) {
  ...
  service()->Init();
  EXPECT_TRUE(IsEnabled(kExtensionId1));
  EXPECT_EQ(extensions::ALLOWLIST_ACKNOWLEDGE_NONE,
  ...
  EXPECT_TRUE(IsDisabled(kExtensionId1));
  EXPECT_EQ(extensions::ALLOWLIST_ACKNOWLEDGE_NEEDED,
            allowlist()->GetExtensionAllowlistAcknowledgeState(kExtensionId1));
}
...
TEST_F(ExtensionAllowlistUnitTest,
  ...
  EXPECT_EQ(extensions::ALLOWLIST_ACKNOWLEDGE_ENABLED_BY_USER,
            allowlist()->GetExtensionAllowlistAcknowledgeState(kExtensionId1));
  EXPECT_TRUE(IsEnabled(kExtensionId1));
  EXPECT_EQ(extensions::ALLOWLIST_NOT_ALLOWLISTED,
            allowlist()->GetExtensionAllowlistState(kExtensionId1));
...
TEST_F(ExtensionAllowlistUnitTest, ReenabledExtensionsAreNotReenforced) {
  ...
  allowlist()->SetExtensionAllowlistAcknowledgeState(
      kExtensionId1, extensions::ALLOWLIST_ACKNOWLEDGE_ENABLED_BY_USER);

  // And an extension that became allowlisted after it was re-enabled by user.
  allowlist()->SetExtensionAllowlistState(kExtensionId2, extensions::ALLOWLIST_ALLOWLISTED);
  allowlist()->SetExtensionAllowlistAcknowledgeState(
      kExtensionId2, extensions::ALLOWLIST_ACKNOWLEDGE_ENABLED_BY_USER);
...
TEST_F(ExtensionAllowlistUnitTest, TurnOffEnhancedProtection) {
  ...
  allowlist()->SetExtensionAllowlistState(kExtensionId3,
                                          extensions::ALLOWLIST_NOT_ALLOWLISTED);
  allowlist()->SetExtensionAllowlistAcknowledgeState(
      kExtensionId3, extensions::ALLOWLIST_ACKNOWLEDGE_ENABLED_BY_USER);
  ...
  // `kExtensionId2`.
  allowlist()->SetExtensionAllowlistAcknowledgeState(
      kExtensionId2, extensions::ALLOWLIST_ACKNOWLEDGE_DONE);
...
TEST_F(ExtensionAllowlistUnitTest, NoEnforcementOnPolicyForceInstall) {
  ...
  {
    ManagementPrefUpdater pref(testing_pref_service());
