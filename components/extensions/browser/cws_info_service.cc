...
bool SaveInfoIfChanged(ExtensionPrefs* extension_prefs,
  ...
  if (!saved_dict || *saved_dict != new_dict) {
    ...
    extension_prefs->SetDictionaryPref(
        id, {kCWSInfo, extensions::kDictionary, PrefScope::kExtensionSpecific},
        std::move(new_dict));
  }
...
// static
CWSInfoService* CWSInfoService::Get(content::BrowserContext* browser_context) {
  return CWSInfoServiceFactory::GetInstance()->GetForBrowserContext(browser_context);
}
...
void CWSInfoService::SetSharedURLLoaderFactoryForTesting(
