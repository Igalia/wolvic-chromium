...
#include "components/extensions/browser/site_permissions_helper.h"
...
constexpr int kRefreshRequiredActionsMask =
    extensions::BLOCKED_ACTION_WEB_REQUEST | extensions::BLOCKED_ACTION_SCRIPT_AT_START;

}  // namespace

SitePermissionsHelper::SitePermissionsHelper(content::BrowserContext* browser_context)
...
SitePermissionsHelper::GetSiteInteraction(
  ...
  if (PermissionsManager::Get(browser_context_)->HasActiveTabAndCanAccess(extension,
                                                                  url)) {
...
void SitePermissionsHelper::UpdateSiteAccess(
  ...
  if (PageNeedsRefreshToRun(blocked_actions)) {
    ...
    runner->ShowReloadPageBubble({extension.id()});
  } else if (blocked_actions != extensions::BLOCKED_ACTION_NONE) {
...
bool SitePermissionsHelper::ShowAccessRequestsInToolbar(
  ...
  ExtensionPrefs::Get(browser_context_)->ReadPrefAsBoolean(
...
void SitePermissionsHelper::SetShowAccessRequestsInToolbar(
  ...
  PermissionsManager::Get(browser_context_)->NotifyShowAccessRequestsInToolbarChanged(
