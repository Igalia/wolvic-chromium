...
std::u16string ExtensionInstallPrompt::Prompt::GetPermissionsHeading() const {
  // TODO(mshin): Replace the below code after migrating Resources
  return u"permission heading message";
  // int id = -1;
  // switch (type_) {
  //   case INSTALL_PROMPT:
  //   case EXTERNAL_INSTALL_PROMPT:
...
ExtensionInstallPrompt::ExtensionInstallPrompt(content::WebContents* contents)
    : browser_context_(contents ? contents->GetBrowserContext() : nullptr),
      extension_(nullptr),
      // TODO(mshin): Support ExtensionInstallUI
      // install_ui_(extensions::CreateExtensionInstallUI(browser_context_.get())),
      show_params_(new ExtensionInstallPromptShowParams(contents)),
      did_call_show_dialog_(false) {}

ExtensionInstallPrompt::ExtensionInstallPrompt(content::BrowserContext* context,
                                               gfx::NativeWindow native_window)
    : browser_context_(context),
      extension_(nullptr),
      // TODO(mshin): Support ExtensionInstallUI
      // install_ui_(extensions::CreateExtensionInstallUI(context)),
      show_params_(
          new ExtensionInstallPromptShowParams(context, native_window)),
      did_call_show_dialog_(false) {}
...
void ExtensionInstallPrompt::LoadImageIfNeeded() {
  // Don't override an icon that was passed in. Also, |browser_context_| can be null in
  // unit tests.
  if (!icon_.empty() || !browser_context_) {
    ShowConfirmation();
    return;
  }
  ...
  extensions::ImageLoader* loader = extensions::ImageLoader::Get(browser_context_);
...
void ExtensionInstallPrompt::ShowConfirmation() {
  ...
  if (custom_permissions_.get()) {
    ...
    permissions_to_display =
        components_extensions::util::GetInstallPromptPermissionSetForExtension(
            extension_.get(), browser_context_, include_optional_permissions);
  }
...
ExtensionInstallPrompt::GetDefaultShowDialogCallback() {
  // TODO(mshin): Move the below code after supporting ExtensionInstallDialogImpl
