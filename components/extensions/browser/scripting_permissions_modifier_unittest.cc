...
#include "components/extensions/browser/extension_service_test_with_install.h"
#include "components/extensions/browser/permissions_test_util.h"
...
namespace util = extensions::util;
...
void InitializeExtensionPermissions(content::BrowserContext* browser_context,
                                    const Extension& extension) {
  PermissionsUpdater updater(browser_context);
...
TEST_F(ScriptingPermissionsModifierUnitTest, GrantAndWithholdHostPermissions) {
  ...
  for (const auto& test_case : test_cases) {
    ...
    PermissionsUpdater(browser_context()).InitializePermissions(extension.get());
    ASSERT_TRUE(
        PermissionsManager::Get(browser_context())->CanAffectExtension(*extension));
...
TEST_F(ScriptingPermissionsModifierUnitTest, WithholdHostPermissionsOnInstall) {
  ...
  PermissionsUpdater(browser_context()).InitializePermissions(extension.get());
  ExtensionPrefs::Get(browser_context())->OnExtensionInstalled(
...
TEST_F(ScriptingPermissionsModifierUnitTest,
  ...
  auto reload_extension = [this, &extension_id]() {
    TestExtensionRegistryObserver observer(ExtensionRegistry::Get(browser_context()));
...
TEST_F(ScriptingPermissionsModifierUnitTest, SwitchBehavior) {
  ...
  PermissionsManager* permissions_manager = PermissionsManager::Get(browser_context());
...
TEST_F(ScriptingPermissionsModifierUnitTest,
  ...
  ScriptingPermissionsModifier modifier(browser_context(), extension.get());
...
TEST_F(ScriptingPermissionsModifierUnitTest, ChangeHostPermissions_AllHosts) {
  ...
  InitializeExtensionPermissions(browser_context(), *extension);
  auto* manager = PermissionsManager::Get(browser_context());
...
TEST_F(ScriptingPermissionsModifierUnitTest,
  ...
  {
    const PermissionsManager::ExtensionSiteAccess site_access =
        PermissionsManager::Get(browser_context())->GetSiteAccess(*extension,
                                                          example_com);
    EXPECT_FALSE(site_access.has_site_access);
    EXPECT_TRUE(site_access.withheld_site_access);
    EXPECT_FALSE(site_access.has_all_sites_access);
...
TEST_F(ScriptingPermissionsModifierUnitTest, AddRuntimeGrantedHostPermission) {
  ...
  ExtensionPrefs::Get(browser_context())->AddRuntimeGrantedPermissions(
      extension->id(),
      PermissionSet(APIPermissionSet(), ManifestPermissionSet(),
  ...
  {
    const PermissionsManager::ExtensionSiteAccess site_access =
        PermissionsManager(browser_context()).GetSiteAccess(*extension, google_com);
...
TEST_F(ScriptingPermissionsModifierUnitTest,
  ...
  EXPECT_FALSE(util::AllowFileAccess(extension->id(), browser_context()));
