...
// #include "components/extensions/browser/pending_extension_manager.h"
...
testing::AssertionResult SharedModuleServiceUnitTest::InstallExtension(
  ...
  service()->OnExtensionInstalled(
      extension, syncer::StringOrdinal(), extensions::kInstallFlagInstallImmediately);
...
// TODO(mshin): Enable the below test after migrating PendingExtensionManager
...
TEST_F(SharedModuleServiceUnitTest, PruneSharedModulesOnUninstall) {
  ...
  service()->UninstallExtension(importing_extension->id(),
                                extensions::UNINSTALL_REASON_FOR_TESTING, &error);
