...
using extensions::GetExtensionFileTaskRunner;
...
WebstoreInstaller::Approval::CreateWithInstallPrompt(BrowserContext* browser_context) {
...
std::unique_ptr<WebstoreInstaller::Approval>
WebstoreInstaller::Approval::CreateForSharedModule(BrowserContext* browser_context) {
  std::unique_ptr<Approval> result(new Approval());
  result->browser_context = browser_context;
...
WebstoreInstaller::WebstoreInstaller(BrowserContext* browser_context,
  ...
  extension_registry_observation_.Observe(ExtensionRegistry::Get(browser_context));
}
...
void WebstoreInstaller::Start() {
  ...
  InstallVerifier::Get(browser_context_)->AddProvisional(ids);
...
void WebstoreInstaller::OnExtensionInstalled(
    ...
    bool is_update) {
  CHECK(ExtensionsBrowserClient::Get()->IsSameContext(browser_context_, browser_context));
...
void WebstoreInstaller::OnDownloadStarted(
  ...
  if (pending_modules_.size() > 1) {
    ...
    std::unique_ptr<Approval> approval =
        Approval::CreateForSharedModule(browser_context_);
...
#else
  base::PathService::Get(base::DIR_ASSETS, &user_data_dir);
#endif
...
void WebstoreInstaller::StartCrxInstaller(const DownloadItem& download) {
  ...
  ExtensionService* service = ExtensionSystem::Get(browser_context_)->
      extension_service();
  CHECK(service);
  ...
  crx_installer_ = download_crx_util::CreateCrxInstaller(browser_context_, download);
