...
#include "components/extensions/browser/safe_browsing/crx_info.pb.h"
...
TEST_F(BlocklistStateFetcherTest, RequestBlocklistState) {
  ...
  EXPECT_EQ(extensions::BLOCKLISTED_SECURITY_VULNERABILITY, result);
}
...
TEST_F(BlocklistStateFetcherTest, RequestMultipleBlocklistStates) {
  ...
  EXPECT_EQ(extensions::NOT_BLOCKLISTED, result[0]);
  EXPECT_EQ(extensions::NOT_BLOCKLISTED, result[1]);
  EXPECT_EQ(extensions::BLOCKLISTED_MALWARE, result[2]);
  EXPECT_EQ(extensions::BLOCKLISTED_MALWARE, result[3]);
  EXPECT_EQ(extensions::BLOCKLISTED_SECURITY_VULNERABILITY, result[4]);
  EXPECT_EQ(extensions::BLOCKLISTED_CWS_POLICY_VIOLATION, result[5]);
  EXPECT_EQ(extensions::BLOCKLISTED_POTENTIALLY_UNWANTED, result[6]);
  EXPECT_EQ(extensions::NOT_BLOCKLISTED, result[7]);
  EXPECT_EQ(extensions::NOT_BLOCKLISTED, result[8]);
}
