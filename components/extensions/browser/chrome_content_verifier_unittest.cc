...
using extensions::VerifierObserver;

namespace {

namespace content_verifier_utils = extensions::content_verifier_utils;
...
class ChromeContentVerifierTest : public ExtensionServiceTestWithInstall {
 public:
  void SetUp() override {
    ...
    // Note: we need a separate TestBrowserContext (other than our base class)
    // because we need it to build |content_verifier_| below in
    // InitContentVerifier().
    testing_browser_context_ = std::make_unique<content::TestBrowserContext>();
  ...
  testing::AssertionResult InstallExtension(const std::string& crx_path_str) {
    ...
    if (!base::PathService::Get(components_extensions::DIR_TEST_DATA, &data_dir))
 ...
 private:
  content::BrowserContext* browser_context() { return testing_browser_context_.get(); }
