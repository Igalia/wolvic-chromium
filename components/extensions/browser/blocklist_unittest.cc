...
#include "components/extensions/browser/test_blocklist_state_fetcher.h"
...
class BlocklistTest : public testing::Test {
 ...
 private:
  ...
  TestExtensionEnvironment env_{Type::kInheritExistingTaskEnvironment};
...
TEST_F(BlocklistTest, OnlyIncludesRequestedIDs) {
  ...
  tester.SetBlocklistState(a, extensions::BLOCKLISTED_MALWARE, false);
  tester.SetBlocklistState(b, extensions::BLOCKLISTED_MALWARE, false);

  EXPECT_EQ(extensions::BLOCKLISTED_MALWARE, tester.GetBlocklistState(a));
  EXPECT_EQ(extensions::BLOCKLISTED_MALWARE, tester.GetBlocklistState(b));
  EXPECT_EQ(extensions::NOT_BLOCKLISTED, tester.GetBlocklistState(c));
...
TEST_F(BlocklistTest, SafeBrowsing) {
  ...
  EXPECT_EQ(extensions::NOT_BLOCKLISTED, tester.GetBlocklistState(a));
...
TEST_F(BlocklistTest, GetBlocklistStates) {
  ...
  tester.SetBlocklistState(a, extensions::BLOCKLISTED_MALWARE, false);
  tester.SetBlocklistState(b, extensions::BLOCKLISTED_SECURITY_VULNERABILITY, false);
  tester.SetBlocklistState(c, extensions::BLOCKLISTED_CWS_POLICY_VIOLATION, false);
  tester.SetBlocklistState(d, extensions::BLOCKLISTED_POTENTIALLY_UNWANTED, false);
  ...
  EXPECT_EQ(extensions::BLOCKLISTED_MALWARE, states_abc[a]);
  EXPECT_EQ(extensions::BLOCKLISTED_SECURITY_VULNERABILITY, states_abc[b]);
  EXPECT_EQ(extensions::BLOCKLISTED_CWS_POLICY_VIOLATION, states_abc[c]);
  EXPECT_EQ(extensions::BLOCKLISTED_SECURITY_VULNERABILITY, states_bcd[b]);
  EXPECT_EQ(extensions::BLOCKLISTED_CWS_POLICY_VIOLATION, states_bcd[c]);
  EXPECT_EQ(extensions::BLOCKLISTED_POTENTIALLY_UNWANTED, states_bcd[d]);
  ...
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(extensions::BLOCKLISTED_MALWARE, states_ad[a]);
  EXPECT_EQ(extensions::BLOCKLISTED_POTENTIALLY_UNWANTED, states_ad[d]);
...
TEST_F(BlocklistTest, FetchBlocklistStates) {
  ...
  EXPECT_EQ(extensions::BLOCKLISTED_CWS_POLICY_VIOLATION, states[a]);
  EXPECT_EQ(extensions::BLOCKLISTED_POTENTIALLY_UNWANTED, states[b]);
  ...
  EXPECT_EQ(extensions::BLOCKLISTED_CWS_POLICY_VIOLATION, cached_states[a]);
  EXPECT_EQ(extensions::BLOCKLISTED_POTENTIALLY_UNWANTED, cached_states[b]);
