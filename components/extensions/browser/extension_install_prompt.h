...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_INSTALL_PROMPT_H_
#define COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_INSTALL_PROMPT_H_
...
#include "components/extensions/browser/install_prompt_permissions.h"
 ...
 public:
  ...
  class Prompt {
   ...
   private:
    ...
    components_extensions::InstallPromptPermissions prompt_permissions_;
  ...
  // Creates a prompt with a context and a native window. The most recently
  // active browser window (or a new browser window if there are no browser
  // windows) is used if a new tab needs to be opened.
  ExtensionInstallPrompt(content::BrowserContext* context, gfx::NativeWindow native_window);
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_INSTALL_PROMPT_H_
