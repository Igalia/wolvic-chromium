...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_BLOCKLIST_H_
#define COMPONENTS_EXTENSIONS_BROWSER_BLOCKLIST_H_
...
class Blocklist : public KeyedService {
 public:
  ...
  using BlocklistStateMap = std::map<extensions::ExtensionId, extensions::BlocklistState>;

  using GetBlocklistedIDsCallback =
      base::OnceCallback<void(const BlocklistStateMap&)>;

  using GetMalwareIDsCallback =
      base::OnceCallback<void(const std::set<extensions::ExtensionId>&)>;

  using IsBlocklistedCallback = base::OnceCallback<void(extensions::BlocklistState)>;
  ...
  void GetBlocklistedIDs(const std::set<extensions::ExtensionId>& ids,
  ...
  // results.
  void GetMalwareIDs(const std::set<extensions::ExtensionId>& ids,
                     GetMalwareIDsCallback callback);

  // More convenient form of GetBlocklistedIDs for checking a single extension.
  void IsBlocklisted(const extensions::ExtensionId& extension_id,
 ...
 private:
  ...
  void GetBlocklistStateForIDs(GetBlocklistedIDsCallback callback,
                               const std::set<extensions::ExtensionId>& blocklisted_ids);

  void RequestExtensionsBlocklistState(const std::set<extensions::ExtensionId>& ids,
                                       base::OnceClosure callback);

  void OnBlocklistStateReceived(const extensions::ExtensionId& id, extensions::BlocklistState state);
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_BLOCKLIST_H_
