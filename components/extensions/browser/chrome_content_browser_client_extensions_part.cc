...
using extensions::ServiceWorkerTaskQueue;
using extensions::URLLoaderFactoryManager;
using extensions::WebViewGuest;
using extensions::WebViewRendererState;
...
const Extension* GetServiceWorkerBasedExtensionForScope(
  ...
  if (!scope.SchemeIs(extensions::kExtensionScheme)) {
    return nullptr;
  }
...
GURL ChromeContentBrowserClientExtensionsPart::GetEffectiveURL(
  ...
  if (url.SchemeIs(extensions::kExtensionScheme) &&
    ...
    return GURL(components_extensions::kExtensionInvalidRequestURL);
  }
...
bool ChromeContentBrowserClientExtensionsPart::ShouldUseProcessPerSite(
    ...
    const GURL& site_url) {
  const Extension* extension =
      GetEnabledExtensionFromSiteURL(browser_context, site_url);
  if (!extension)
    return false;
  ...
  if (extension->GetType() == extensions::Manifest::TYPE_HOSTED_APP) {
    if (!extension->permissions_data()->HasAPIPermission(
            extensions::mojom::APIPermissionID::kBackground) ||
...
bool ChromeContentBrowserClientExtensionsPart::
  ...
  bool is_current_url_for_webstore_app =
      current_extension && current_extension->id() == extensions::kWebStoreAppId;

  const Extension* dest_extension =
      registry->enabled_extensions().GetExtensionOrAppByURL(
          destination_effective_url);
  bool is_dest_url_for_webstore_app =
      dest_extension && dest_extension->id() == extensions::kWebStoreAppId;
...
bool ChromeContentBrowserClientExtensionsPart::AllowServiceWorker(
  ...
  // We only care about extension urls.
  if (!first_party_url.SchemeIs(extensions::kExtensionScheme))
    return true;

  const Extension* extension = ExtensionRegistry::Get(context)
                                   ->enabled_extensions()
                                   .GetExtensionOrAppByURL(first_party_url);
  return ::components_extensions::AllowServiceWorker(scope, script_url, extension);
}
