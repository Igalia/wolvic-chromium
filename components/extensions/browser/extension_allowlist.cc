...
using extensions::AllowlistState;
using extensions::ExtensionsBrowserClient;
using extensions::ExtensionId;
using extensions::ExtensionPrefs;
using extensions::ExtensionRegistry;
using extensions::ExtensionSet;
using extensions::PrefMap;
using extensions::PrefScope;
using extensions::PrefType;
...
ExtensionAllowlist::ExtensionAllowlist(BrowserContext* browser_context,
  ...
  // enforcements.
  pref_change_registrar_.Init(ExtensionsBrowserClient::Get()->GetPrefServiceForContext(browser_context));
...
AllowlistState ExtensionAllowlist::GetExtensionAllowlistState(
  ...
  if (!extension_prefs_->ReadPrefAsInteger(extension_id, kPrefAllowlist,
                                           &value)) {
    return extensions::ALLOWLIST_UNDEFINED;
  }

  if (value < 0 || value > extensions::ALLOWLIST_LAST) {
...
void ExtensionAllowlist::SetExtensionAllowlistState(
  ...
  DCHECK_NE(state, extensions::ALLOWLIST_UNDEFINED);
  ...
  if (warnings_enabled_) {
    NotifyExtensionAllowlistWarningStateChanged(
        extension_id, /*show_warning=*/state == extensions::ALLOWLIST_NOT_ALLOWLISTED);
  }
}

extensions::AllowlistAcknowledgeState
ExtensionAllowlist::GetExtensionAllowlistAcknowledgeState(
  ...
  if (!extension_prefs_->ReadPrefAsInteger(extension_id,
    ...
    return extensions::ALLOWLIST_ACKNOWLEDGE_NONE;
  }

  if (value < 0 || value > extensions::ALLOWLIST_ACKNOWLEDGE_LAST) {
    LOG(ERROR) << "Bad pref 'allowlist_acknowledge' for extension '"
               << extension_id << "'";
    return extensions::ALLOWLIST_ACKNOWLEDGE_NONE;
  }

  return static_cast<extensions::AllowlistAcknowledgeState>(value);
}
...
void ExtensionAllowlist::PerformActionBasedOnOmahaAttributes(
  ...
  if (!allowlist_value) {
    ...
    // extensions in the |extensions::ALLOWLIST_UNDEFINED| state.
    return;
  }

  AllowlistState allowlist_state = allowlist_value->GetBool()
                                       ? extensions::ALLOWLIST_ALLOWLISTED
                                       : extensions::ALLOWLIST_NOT_ALLOWLISTED;
    ...
    if (allowlist_state == extensions::ALLOWLIST_ALLOWLISTED) {
      ...
      if (registry_->enabled_extensions().Contains(extension_id)) {
        ...
        SetExtensionAllowlistAcknowledgeState(extension_id,
                                              extensions::ALLOWLIST_ACKNOWLEDGE_NEEDED);
      }
    } else {
...
bool ExtensionAllowlist::ShouldDisplayWarning(
  ...
  if (GetExtensionAllowlistState(extension_id) != extensions::ALLOWLIST_NOT_ALLOWLISTED)
...
void ExtensionAllowlist::OnExtensionInstalled(const ExtensionId& extension_id,
  ...
  if (install_flags & extensions::kInstallFlagBypassedSafeBrowsingFriction) {
    SetExtensionAllowlistAcknowledgeState(
        extension_id, extensions::ALLOWLIST_ACKNOWLEDGE_ENABLED_BY_USER);
    SetExtensionAllowlistState(extension_id, extensions::ALLOWLIST_NOT_ALLOWLISTED);
  }
}
...
void ExtensionAllowlist::ApplyEnforcement(const ExtensionId& extension_id) {
  ...
  if (GetExtensionAllowlistAcknowledgeState(extension_id) ==
      extensions::ALLOWLIST_ACKNOWLEDGE_ENABLED_BY_USER) {
    return;
  }
  ...
  if (was_enabled &&
    ...
    SetExtensionAllowlistAcknowledgeState(extension_id,
                                          extensions::ALLOWLIST_ACKNOWLEDGE_NONE);
  }
}
...
void ExtensionAllowlist::OnExtensionStateChanged(
  ...
  // `extensions::ALLOWLIST_ACKNOWLEDGE_ENABLED_BY_USER` instead of disabled.
  if (!init_done_)
    return;
