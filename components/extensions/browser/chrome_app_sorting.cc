...
// TODO(mshin): Enable below codes after supporting Sync
// #include "components/extensions/browser/extension_sync_service.h"
...
using extensions::AppDisplayInfo;
...
void ChromeAppSorting::InitializePageOrdinalMapFromWebApps() {
  ...
  // auto* web_app_provider = web_app::WebAppProvider::GetForWebApps(browser_context_);
  // if (!web_app_provider)
  //   return;
...
void ChromeAppSorting::InitializePageOrdinalMap(
  ...
  for (auto ext_it = extension_ids.begin(); ext_it != extension_ids.end();
    ...
    DCHECK(*ext_it != kChromeAppId);
  }

  // Include the Web Store App since it is displayed on the NTP.
  syncer::StringOrdinal web_store_app_page = GetPageOrdinal(extensions::kWebStoreAppId);
  if (web_store_app_page.IsValid()) {
    AddOrdinalMapping(extensions::kWebStoreAppId, web_store_app_page,
  ...
  syncer::StringOrdinal chrome_app_page =
      GetPageOrdinal(kChromeAppId);
  if (chrome_app_page.IsValid()) {
    AddOrdinalMapping(kChromeAppId, chrome_app_page,
                      GetAppLaunchOrdinal(kChromeAppId));
  }
}
...
void ChromeAppSorting::SyncIfNeeded(const ExtensionId& extension_id) {
  ...
  // TODO(mshin): Enable the below code after migrating ExtensionSyncService
  // ExtensionRegistry* registry = ExtensionRegistry::Get(browser_context_);
  // const Extension* extension = registry->GetInstalledExtension(extension_id);
  // if (extension) {
  //   ExtensionSyncService::Get(browser_context_)->SyncExtensionChangeIfNeeded(*extension);
  // }
}
