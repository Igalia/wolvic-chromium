...
using extensions::PreloadCheckGroup;
using extensions::PolicyCheck;
using extensions::RequirementsChecker;
...
void MaybeCleanupMetadataFolder(const base::FilePath& extension_path) {
  const std::vector<base::FilePath> reserved_filepaths =
      extensions::file_util::GetReservedMetadataFilePaths(extension_path);
  for (const auto& file : reserved_filepaths)
    base::DeletePathRecursively(file);

  const base::FilePath& metadata_dir = extension_path.Append(extensions::kMetadataFolder);
...
bool UnpackedInstaller::LoadFromCommandLine(const base::FilePath& path_in,
  ...
  extension_path_ =
      base::MakeAbsoluteFilePath(extensions::path_util::ResolveHomeDirectory(path_in));
  ...
  std::string error;
  if (!LoadExtension(extensions::mojom::ManifestLocation::kCommandLine, GetFlags(),
                     &error)) {
...
void UnpackedInstaller::StartInstallChecks() {
  ...
  if (extension()->manifest()->location() !=
    ...
    if (SharedModuleInfo::ImportsModules(extension())) {
      ...
      ExtensionRegistry* registry = ExtensionRegistry::Get(service->GetBrowserContext());
...
int UnpackedInstaller::GetFlags() {
  ...
  bool allow_file_access =
      Manifest::ShouldAlwaysAllowFileAccess(extensions::mojom::ManifestLocation::kUnpacked);
  ExtensionPrefs* prefs = ExtensionPrefs::Get(service_weak_->GetBrowserContext());
...
bool UnpackedInstaller::LoadExtension(extensions::mojom::ManifestLocation location,
                                      int flags,
                                      std::string* error) {
  ...
  if (location == extensions::mojom::ManifestLocation::kUnpacked &&
      !extensions::file_util::CheckForIllegalFilenames(extension_path_, error)) {
    return false;
  }

  extension_ =
      extensions::file_util::LoadExtension(extension_path_, location, flags, error);
...
bool UnpackedInstaller::IndexAndPersistRulesIfNeeded(std::string* error) {
  ...
  auto ruleset_filter = extensions::declarative_net_request::FileBackedRulesetSource::
      RulesetFilter::kIncludeAll;
  auto parse_flags =
      extensions::declarative_net_request::RulesetSource::kRaiseErrorOnInvalidRules |
      extensions::declarative_net_request::RulesetSource::kRaiseWarningOnLargeRegexRules;
  ...
  // browser process.
  extensions::declarative_net_request::InstallIndexHelper::Result result =
      extensions::declarative_net_request::InstallIndexHelper::IndexStaticRulesetsUnsafe(
...
bool UnpackedInstaller::IsLoadingUnpackedAllowed() const {
  ...
  return !ExtensionManagementFactory::GetForBrowserContext(
              service_weak_->GetBrowserContext())
...
void UnpackedInstaller::LoadWithFileAccess(int flags) {
  std::string error;
  if (!LoadExtension(extensions::mojom::ManifestLocation::kUnpacked, flags, &error)) {
    // Set priority explicitly to avoid unwanted task priority inheritance.
    content::GetUIThreadTaskRunner({base::TaskPriority::USER_BLOCKING})
        ->PostTask(FROM_HERE,
...
void UnpackedInstaller::ReportExtensionLoadError(const std::string &error) {
  ...
  if (service_weak_.get()) {
    LoadErrorReporter::GetInstance()->ReportLoadError(
        extension_path_, error, service_weak_->GetBrowserContext(), be_noisy_on_failure_);
  }
