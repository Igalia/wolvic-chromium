...
using extensions::IncognitoInfo;
...
bool SkipDeleteExtensionDir(const Extension& extension,
  ...
  bool extension_dir_not_direct_subdir_of_unpacked_extensions_install_dir =
      ...
      browser_context_path.AppendASCII(extensions::kUnpackedInstallDirectoryName);
...
void ExtensionService::CheckExternalUninstall(const std::string& id) {
  ...
  UninstallExtension(id, extensions::UNINSTALL_REASON_ORPHANED_EXTERNAL_EXTENSION, nullptr);
}
...
void ExtensionService::GreylistExtensionForTest(
  ...
  extensions::blocklist_prefs::SetSafeBrowsingExtensionBlocklistState(extension_id, state,
                                                          extension_prefs_);
...
bool ExtensionService::OnExternalExtensionUpdateUrlFound(
  ...
  if (extension) {
    ...
    extensions::mojom::ManifestLocation current = extension->location();
    ...
    if (current !=
      ...
      UnloadExtension(info.extension_id, extensions::UnloadedExtensionReason::UPDATE);
      ...
      corrupted_extension_reinstaller_(browser_context_)
{
  ...
  // TODO(mshin): Support browser_shutdown
  // on_app_terminating_subscription_ =
  //     browser_shutdown::AddAppTerminatingCallback(base::BindOnce(
  //         &ExtensionService::OnAppTerminating, base::Unretained(this)));

  host_registry_observation_.Observe(ExtensionHostRegistry::Get(context));
  ...
  //       this, browser_context_, &external_extension_providers_);
  // }
...
scoped_refptr<CrxInstaller> ExtensionService::CreateUpdateInstaller(
  ...
  // TODO(mshin): Enable the below code after migrating PendingExtensionInfo
  ...
  //       ExtensionPrefs::Get(browser_context_)->HasDisableReason(
...
bool ExtensionService::UninstallExtension(
  ...
  bool external_uninstall =
      ...
      (reason == extensions::UNINSTALL_REASON_MIGRATED) ||
  ...
  if (!external_uninstall &&
    ...
    ExtensionRegistry::Get(browser_context_)->TriggerOnUninstallationDenied(
        extension.get());
    return false;
  }
  ...
  UnloadExtension(extension->id(), extensions::UnloadedExtensionReason::UNINSTALL);
  ...
  if (!SkipDeleteExtensionDir(*extension, browser_context_->GetPath())) {
    ...
    // thread.
    if (!extensions::GetExtensionFileTaskRunner()->PostTaskAndReply(
            FROM_HERE,
            base::BindOnce(&ExtensionService::UninstallExtensionOnFileThread,
                           extension->id(), /* profile_user_name */"",
  ...
  // TODO(mshin): Enable the below code after migrating DataDeleter
  // DataDeleter::StartDeleting(browser_context_, extension.get(), subtask_done_callback);

  extension_registrar_.UntrackTerminatedExtension(extension->id());

  // Notify interested parties that we've uninstalled this extension.
  ExtensionRegistry::Get(browser_context_)->TriggerOnUninstalled(extension.get(),
                                                         reason);
...
void ExtensionService::OnGreylistStateRemoved(const std::string& extension_id) {
  ...
  bool is_on_omaha_list =
      extensions::blocklist_prefs::HasAnyOmahaGreylistState(extension_id, extension_prefs_);
  ...
  extensions::blocklist_prefs::ClearAcknowledgedGreylistStates(extension_id,
                                                   extension_prefs_);
...
void ExtensionService::OnGreylistStateAdded(const std::string& extension_id,
  ...
  bool has_new_state_on_sb_list =
      (extensions::blocklist_prefs::GetSafeBrowsingExtensionBlocklistState(
           extension_id, extension_prefs_) == new_state);
  bool has_new_state_on_omaha_list = extensions::blocklist_prefs::HasOmahaBlocklistState(
      extension_id, new_state, extension_prefs_);
  DCHECK(has_new_state_on_sb_list || has_new_state_on_omaha_list);
#endif
  if (extensions::blocklist_prefs::HasAcknowledgedBlocklistState(extension_id, new_state,
                                                     extension_prefs_)) {
  ...
  extensions::blocklist_prefs::UpdateCurrentGreylistStatesAsAcknowledged(extension_id,
                                                             extension_prefs_);
  DisableExtension(extension_id, extensions::disable_reason::DISABLE_GREYLIST);
}

void ExtensionService::OnBlocklistStateRemoved(
    const std::string& extension_id) {
  if (extensions::blocklist_prefs::IsExtensionBlocklisted(extension_id, extension_prefs_)) {
    return;
  }

  // Clear acknowledged state.
  extensions::blocklist_prefs::RemoveAcknowledgedBlocklistState(
      extension_id, BitMapBlocklistState::BLOCKLISTED_MALWARE,
      extension_prefs_);
...
void ExtensionService::OnBlocklistStateAdded(const std::string& extension_id) {
  DCHECK(
      extensions::blocklist_prefs::IsExtensionBlocklisted(extension_id, extension_prefs_));
  // The extension was already acknowledged by the user, it should already be in
  // the unloaded state.
  if (extensions::blocklist_prefs::HasAcknowledgedBlocklistState(
          extension_id, BitMapBlocklistState::BLOCKLISTED_MALWARE,
          extension_prefs_)) {
  ...
  UnloadExtension(extension_id, extensions::UnloadedExtensionReason::BLOCKLIST);
}

void ExtensionService::RemoveDisableReasonAndMaybeEnable(
    const std::string& extension_id,
    extensions::disable_reason::DisableReason reason_to_remove) {
...
void ExtensionService::DisableExtensionWithSource(
  ...
  DCHECK(disable_reasons == extensions::disable_reason::DISABLE_USER_ACTION ||
         disable_reasons == extensions::disable_reason::DISABLE_BLOCKED_BY_POLICY);
...
void ExtensionService::RecordPermissionMessagesHistogram(
    ...
    bool log_user_browser_context_histograms) {
  ...
  if (log_user_browser_context_histograms) {
    base::UmaHistogramBoolean(
...
void ExtensionService::PostActivateExtension(
  ...
  // TODO(mshin): Enable the below code after supporting ExtensionSpecialStoragePolicy
  // browser_context_->GetExtensionSpecialStoragePolicy()->GrantRightsForExtension(
  //     extension.get(), browser_context_);
  ...
  DCHECK(permissions_data);

  // If the extension has permission to load chrome://favicon/ resources we need
  // to make sure that the FaviconSource is registered with the
  // ChromeURLDataManager.
  // TODO(mshin): Enable the below code after supporting FaviconSource
  // if (permissions_data->HasHostPermission(GURL(kChromeUIFaviconURL))) {
  //   content::URLDataSource::Add(
  //       browser_context_, std::make_unique<FaviconSource>(
  //                     browser_context_, chrome::FaviconUrlFormat::kFaviconLegacy));
  // }

  // Same for chrome://theme/ resources.
  // TODO(mshin): Enable the below code after supporting ThemeSource
  // if (permissions_data->HasHostPermission(GURL(kChromeUIThemeURL))) {
  //   content::URLDataSource::Add(browser_context_,
  //                               std::make_unique<ThemeSource>(browser_context_));
  // }
}
...
void ExtensionService::PostDeactivateExtension(
  ...
  // BrowserContextKeyedService and use ExtensionRegistryObserver.
  // TODO(mshin): Enable the below code after supporting ExtensionSpecialStoragePolicy
  // browser_context_->GetExtensionSpecialStoragePolicy()->RevokeRightsForExtension(
  //     extension.get(), browser_context_);
...
void ExtensionService::CheckManagementPolicy() {
  ...
  for (const auto& extension : registry_->enabled_extensions()) {
    extensions::disable_reason::DisableReason disable_reason = extensions::disable_reason::DISABLE_NONE;
  ...
  for (const auto& extension : registry_->disabled_extensions()) {
    ...
    if (management->CheckMinimumVersion(extension.get(), nullptr)) {
      disable_reasons &= (~extensions::disable_reason::DISABLE_UPDATE_REQUIRED_BY_POLICY);
    }
    ...
    if (management->IsAllowedByUnpublishedAvailabilityPolicy(extension.get())) {
      disable_reasons &=
          ~extensions::disable_reason::DISABLE_PUBLISHED_IN_STORE_REQUIRED_BY_POLICY;
    } else {
      disable_reasons |=
          extensions::disable_reason::DISABLE_PUBLISHED_IN_STORE_REQUIRED_BY_POLICY;
    }
    ...
    // Remove any supervised user related disable reasons.
    disable_reasons &= (~extensions::disable_reason::DISABLE_CUSTODIAN_APPROVAL_REQUIRED);
    ...
    if (system_->management_policy()->MustRemainEnabled(extension.get(),
      ...
      disable_reasons &= (~extensions::disable_reason::DISABLE_EXTERNAL_EXTENSION);
    }
  ...
  for (auto extension_id : remove_list) {
    std::u16string error;
    if (!UninstallExtension(extension_id, extensions::UNINSTALL_REASON_INTERNAL_MANAGEMENT,
                            &error)) {
...
void ExtensionService::RemoveComponentExtension(
  ...
  UnloadExtension(extension_id, extensions::UnloadedExtensionReason::UNINSTALL);
  if (extension.get()) {
    ExtensionRegistry::Get(browser_context_)->TriggerOnUninstalled(
...
void ExtensionService::AddExtension(const Extension* extension) {
  ...
  if (registry_->disabled_extensions().Contains(extension->id())) {
    ...
    const int kReasonMask = extensions::disable_reason::DISABLE_PERMISSIONS_INCREASE |
                            extensions::disable_reason::DISABLE_REMOTE_INSTALL;
    if (reasons & kReasonMask && !(reasons & ~kReasonMask)) {
      // TODO(mshin): Enable the below code after migrating extension_disabled_ui
...
void ExtensionService::AddComponentExtension(const Extension* extension) {
  ...
  if (!old_version.IsValid() || old_version != extension->version()) {
    ...
    AddNewOrUpdatedExtension(extension, Extension::ENABLED, extensions::kInstallFlagNone,
...
void ExtensionService::CheckPermissionsIncrease(const Extension* extension,
  ...
  if (disable_reasons & extensions::disable_reason::DISABLE_PERMISSIONS_INCREASE &&
      !is_privilege_increase) {
    disable_reasons &= ~extensions::disable_reason::DISABLE_PERMISSIONS_INCREASE;
    extension_prefs_->RemoveDisableReason(
        extension->id(), extensions::disable_reason::DISABLE_PERMISSIONS_INCREASE);
  }
...
void ExtensionService::OnExtensionInstalled(
    ...
    int install_flags,
    const extensions::declarative_net_request::RulesetInstallPrefs& ruleset_install_prefs) {
  CHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  ...
  // TODO(mshin): Enable the below code after migrating PendingExtensionManager
  // if (pending_extension_info) {
  //   if (!pending_extension_info->ShouldAllowInstall(extension, GetBrowserContext())) {
  //     // Hack for crbug.com/558299, see comment on DeleteThemeDoNotUse.
  //     if (extension->is_theme() && pending_extension_info->is_from_sync())
  //       ExtensionSyncService::Get(browser_context_)->DeleteThemeDoNotUse(*extension);
  ...
  if (install_flags & extensions::kInstallFlagHasRequirementErrors) {
    disable_reasons |= extensions::disable_reason::DISABLE_UNSUPPORTED_REQUIREMENT;
  } else {
  ...
  if (ExtensionManagementFactory::GetForBrowserContext(GetBrowserContext())
          ->CheckMinimumVersion(extension, nullptr)) {
    // And remove the corresponding disable reason.
    disable_reasons &= ~extensions::disable_reason::DISABLE_UPDATE_REQUIRED_BY_POLICY;
  }

  if (install_flags & extensions::kInstallFlagIsBlocklistedForMalware) {
  ...
  const Extension::State initial_state =
      disable_reasons == extensions::disable_reason::DISABLE_NONE ? Extension::ENABLED
                                                      : Extension::DISABLED;
...
bool ExtensionService::OnExternalExtensionFileFound(
  ...
  if (existing) {
    ...
    bool is_preinstalled_apps_migration =
        (info.crx_location == extensions::mojom::ManifestLocation::kInternal &&
  ...
  CRXFileInfo file_info(
      info.path, info.crx_location == extensions::mojom::ManifestLocation::kExternalPolicy
...
bool ExtensionService::CanBlockExtension(const Extension* extension) const {
  DCHECK(extension);
  return extension->location() != extensions::mojom::ManifestLocation::kComponent &&
         extension->location() != extensions::mojom::ManifestLocation::kExternalComponent &&
...
bool ExtensionService::CanDisableExtension(const Extension* extension) {
  ...
  //   preferences.
  if (extension->location() == extensions::mojom::ManifestLocation::kExternalComponent)
    return true;
...
void ExtensionService::UnloadAllExtensionsInternal() {
  // TODO(mshin): Enable the below code after supporting ExtensionSpecialStoragePolicy
  // browser_context_->GetExtensionSpecialStoragePolicy()->RevokeRightsForAllExtensions();
...
void ExtensionService::UninstallMigratedExtensions() {
  ...
  for (const std::string& extension_id : kObsoleteComponentExtensionIds) {
    ...
    if (extension) {
      UninstallExtension(extension_id, extensions::UNINSTALL_REASON_COMPONENT_REMOVED,
                         nullptr);
