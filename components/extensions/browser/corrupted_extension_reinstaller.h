...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_CORRUPTED_EXTENSION_REINSTALLER_H_
#define COMPONENTS_EXTENSIONS_BROWSER_CORRUPTED_EXTENSION_REINSTALLER_H_
...
class CorruptedExtensionReinstaller {
 public:
  ...
  void ExpectReinstallForCorruption(
      ...
      extensions::mojom::ManifestLocation manifest_location_for_uma);

  // Call this method when extension in reinstalled to remove it from the set
  // and update the metrics.
  void MarkResolved(const extensions::ExtensionId& id);

  // Returns true if we are expecting a reinstall of the extension with |id| due
  // to corruption?
  bool IsReinstallForCorruptionExpected(const extensions::ExtensionId& id) const;
  ...
  const std::map<extensions::ExtensionId, base::TimeTicks>& GetExpectedReinstalls() const;
 ...
 private:
  ...
  std::map<extensions::ExtensionId, base::TimeTicks> expected_reinstalls_;
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_CORRUPTED_EXTENSION_REINSTALLER_H_
