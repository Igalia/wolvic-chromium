...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_SERVICE_TEST_WITH_INSTALL_H_
#define COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_SERVICE_TEST_WITH_INSTALL_H_
...
class ExtensionServiceTestWithInstall : public ExtensionServiceUserTestBase,
 ...
 protected:
  ...
  PackAndInstallCRX(const base::FilePath& dir_path,
                    const base::FilePath& pem_path,
                    InstallState install_state,
                    int creation_flags,
                    extensions::mojom::ManifestLocation install_location);
  const extensions::Extension*
  PackAndInstallCRX(const base::FilePath& dir_path,
                    const base::FilePath& pem_path,
                    InstallState install_state);
  ...
  PackAndInstallCRX(const base::FilePath& dir_path,
                    ...
                    InstallState install_state);
  const extensions::Extension*
  InstallCRX(const base::FilePath& path,
             InstallState install_state,
             int creation_flags,
  ...
  InstallCRX(const base::FilePath& path,
             InstallState install_state);
  const extensions::Extension*
  InstallCRXFromWebStore(const base::FilePath& path,
                         InstallState install_state);
  ...
  // Returns an extensions::Extension pointer if the install succeeded, null otherwise.
  const extensions::Extension*
  VerifyCrxInstall(const base::FilePath& path,
                   InstallState install_state);
  ...
  const extensions::ExtensionList& loaded_extensions() const {
    return loaded_extensions_;
  }
  const extensions::Extension* installed_extension() const {
    return installed_extension_;
  }
  bool was_update() const { return was_update_; }
  extensions::UnloadedExtensionReason unloaded_reason() const { return unloaded_reason_; }
 ...
 private:
  ...
  raw_ptr<const extensions::Extension, DanglingUntriaged> installed_extension_;
  bool was_update_;
  std::string old_name_;
  std::string unloaded_id_;
  extensions::UnloadedExtensionReason unloaded_reason_;
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_SERVICE_TEST_WITH_INSTALL_H_
