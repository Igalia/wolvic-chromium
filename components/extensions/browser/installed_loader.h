...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_INSTALLED_LOADER_H_
#define COMPONENTS_EXTENSIONS_BROWSER_INSTALLED_LOADER_H_
...
class InstalledLoader {
 public:
  ...
  void Load(const extensions::ExtensionInfo& info, bool write_to_prefs);
  ...
  void RecordExtensionsIncrementedMetricsForTesting(content::BrowserContext* context);

 private:
  // Returns the flags that should be used with Extension::Create() for an
  // extension that is already installed.
  int GetCreationFlags(const extensions::ExtensionInfo* info);
  ...
  void RecordExtensionsMetrics(content::BrowserContext* context, bool is_user_profile);
