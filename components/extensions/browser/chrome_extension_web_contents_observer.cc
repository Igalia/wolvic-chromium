...
void ChromeExtensionWebContentsObserver::RenderFrameCreated(
  ...
  extensions::ExtensionWebContentsObserver::RenderFrameCreated(render_frame_host);
  ...
  if ((extension->is_extension() || extension->is_platform_app()) &&
    ...
    // TODO(mshin) : Enable the below code after migrating Theme
    // policy->GrantRequestOrigin(
  ...
  if (extension->is_extension() ||
      ...
      (extension->is_platform_app() &&
       Manifest::IsComponentLocation(extension->location()))) {
    policy->GrantRequestOrigin(
        process_id, url::Origin::Create(GURL(kChromeUIFaviconURL)));
    policy->GrantRequestOrigin(
        process_id,
        url::Origin::Create(GURL(kChromeUIExtensionIconURL)));
  }
}
...
void ChromeExtensionWebContentsObserver::InitializeRenderFrame(
    content::RenderFrameHost* render_frame_host) {
  DCHECK(initialized());
  extensions::ExtensionWebContentsObserver::InitializeRenderFrame(render_frame_host);
  // Enable the below code after migrating WindowController
...
void ChromeExtensionWebContentsObserver::ReloadIfTerminated(
    content::RenderFrameHost* render_frame_host) {
  DCHECK(initialized());
  std::string extension_id = extensions::util::GetExtensionIdFromFrame(render_frame_host);
