...
namespace declarative_net_request = extensions::declarative_net_request;
...
ExtensionAction::ShowAction ExtensionActionRunner::RunAction(
  ...
  // TODO(mshin): Enable the below code after supporting the side panel
...
void ExtensionActionRunner::GrantTabPermissions(
  ...
  // TODO(mshin): Enable the below code after migrating SitePermissionsHelper
...
int ExtensionActionRunner::GetBlockedActions(
  ...
  int blocked_actions = extensions::BLOCKED_ACTION_NONE;
  if (web_request_blocked_.count(extension_id) != 0)
    blocked_actions |= extensions::BLOCKED_ACTION_WEB_REQUEST;
    ...
    for (const auto& script : iter->second) {
      switch (script->run_location) {
        case mojom::RunLocation::kDocumentStart:
          blocked_actions |= extensions::BLOCKED_ACTION_SCRIPT_AT_START;
          break;
        case mojom::RunLocation::kDocumentEnd:
        case mojom::RunLocation::kDocumentIdle:
        case mojom::RunLocation::kBrowserDriven:
          blocked_actions |= extensions::BLOCKED_ACTION_SCRIPT_OTHER;
          break;
...
bool ExtensionActionRunner::WantsToRun(const Extension* extension) {
  return GetBlockedActions(extension->id()) != extensions::BLOCKED_ACTION_NONE;
}
...
void ExtensionActionRunner::ShowReloadPageBubble(
  ...
  // TODO(mshin): Enable the below code after supporting Bubble View
