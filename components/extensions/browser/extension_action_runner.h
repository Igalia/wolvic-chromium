...
#ifndef COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_ACTION_RUNNER_H_
#define COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_ACTION_RUNNER_H_
...
class ExtensionActionRunner : public content::WebContentsObserver,
 ...
 public:
  ...
  extensions::ExtensionAction::ShowAction
  RunAction(const extensions::Extension* extension,
  ...
  void RunBlockedActions(const extensions::Extension* extension);
  ...
  void OnRequestScriptInjectionPermission(
      ...
      extensions::mojom::LocalFrameHost::RequestScriptInjectionPermissionCallback callback);
  ...
  RequiresUserConsentForScriptInjectionForTesting(
      const extensions::Extension* extension,
      extensions::mojom::InjectionType type) {
    return RequiresUserConsentForScriptInjection(extension, type);
  }
  void RequestScriptInjectionForTesting(const extensions::Extension* extension,
                                        extensions::mojom::RunLocation run_location,
                                        ScriptInjectionCallback callback) {
    return RequestScriptInjection(extension, run_location, std::move(callback));
  }
  void ClearInjectionsForTesting(const extensions::Extension& extension) {
 ...
 private:
  ...
  struct PendingScript {
    PendingScript(extensions::mojom::RunLocation run_location,
  ...
  using PendingScriptMap = std::map<extensions::ExtensionId, PendingScriptList>;
  ...
  extensions::PermissionsData::PageAccess
  RequiresUserConsentForScriptInjection(
      const extensions::Extension* extension,
      extensions::mojom::InjectionType type);
  ...
  void RequestScriptInjection(const extensions::Extension* extension,
                              extensions::mojom::RunLocation run_location,
                              ScriptInjectionCallback callback);

  // Runs any pending injections for the corresponding extension.
  void RunPendingScriptsForExtension(const extensions::Extension* extension);

  // Notifies the ExtensionActionAPI of a change (either that an extension now
  // wants permission to run, or that it has been run).
  void NotifyChange(const extensions::Extension* extension);
  ...
  std::set<extensions::ExtensionId> web_request_blocked_;
  ...
  std::set<extensions::ExtensionId> permitted_extensions_;
...
#endif  // COMPONENTS_EXTENSIONS_BROWSER_EXTENSION_ACTION_RUNNER_H_
