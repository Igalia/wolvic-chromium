...
using extensions::APIBindingHooks;
using extensions::APISignature;
using extensions::APITypeReferenceMap;
using extensions::Extension;
using extensions::ExtensionsRendererClient;
using extensions::ExtensionFrameHelper;
using extensions::Message;
using extensions::MessageTarget;
using extensions::NativeRendererMessagingService;
using extensions::RuntimeHooksDelegate;
using extensions::ScriptContext;

namespace components_extensions {

namespace binding = extensions::binding;
namespace messaging_util = extensions::messaging_util;
...
RequestResult ExtensionHooksDelegate::HandleRequest(
  ...
  ScriptContext* script_context = extensions::GetScriptContextFromV8ContextChecked(context);
  ...
  if (!arguments[0]->IsNull()) {
    ...
    if (!v8_view_type->IsUndefined()) {
      ...
      if (view_type_string != "ALL") {
        bool success = extensions::GetViewTypeFromString(view_type_string, &view_type);
        DCHECK(success);
      }
    }
  }
