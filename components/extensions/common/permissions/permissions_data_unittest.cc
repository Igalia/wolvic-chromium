...
namespace switches = extensions::switches;
...
GURL GetFaviconURL(const char* path) {
  ...
  return GURL(kChromeUIFaviconURL).ReplaceComponents(replace_path);
}
...
void CheckRestrictedUrls(const Extension* extension,
  ...
  const GURL chrome_settings_url(kChromeUISettingsURL);
 ...
 protected:
  ExtensionScriptAndCaptureVisibleTest()
      : http_url("http://www.google.com"),
        ...
        settings_url(kChromeUISettingsURL),
...
TEST_F(CaptureVisiblePageTest, PolicyBlockedURLs) {
  ...
  const GURL test_urls[] = {
      GURL("https://example.com"), GURL(kChromeUISettingsURL),
  };
