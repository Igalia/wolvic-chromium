...
using components_extensions::ChromeExtensionsClient;
using extensions::ContextData;
...
TEST_F(ChromeExtensionsClientTest, GetBrowserImagePaths) {
  ...
  ASSERT_TRUE(base::PathService::Get(components_extensions::DIR_TEST_DATA, &install_dir));
...
TEST_F(ChromeExtensionsClientTest, CheckManifestHandlerRegistryForOverflow) {
  extensions::ManifestHandlerRegistry* registry = extensions::ManifestHandlerRegistry::Get();
  ASSERT_TRUE(registry);
  ASSERT_LT(0u, registry->handlers_.size());
  EXPECT_LE(registry->handlers_.size(), extensions::ManifestHandlerRegistry::kHandlerMax);
}
